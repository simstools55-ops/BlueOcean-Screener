...
- Homeステータス更新をCandidate Pool同期後の1回に集約し、背景画面の再描画を抑制。
- 判定ロジックは変更なし。
...
- 実在確認済み検索証拠の最低件数を8件から5件へ変更。
- 対象サイト自身の記事URLをSERP証拠で検出した場合、`EXISTING_ARTICLE`（既存記事確認済み）として新規記事候補から除外。
- 既存記事確認済み候補は履歴に残し、GREEN＋YELLOW目標件数には加算しない。

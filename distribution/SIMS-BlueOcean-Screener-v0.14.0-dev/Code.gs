...
function sbosSyncCandidatePool_(){
  ...
  if(pool.getLastRow()>=2)pool.getRange(2,1,pool.getLastRow()-1,1).insertCheckboxes();return {upserted:upserted,removed:removed};
}
function sbosOpenCandidatePool(){sbosEnsureSheets_();sbosSyncCandidatePool_();const sh=SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATE_POOL);SpreadsheetApp.getActive().setActiveSheet(sh);sh.setActiveSelection('A1');}
function sbosGetSelectedPoolCandidate_(){const sh=SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATE_POOL);if(!sh||sh.getLastRow()<2)return null;const v=sh.getRange(2,1,sh.getLastRow()-1,12).getValues();const a=v.map((r,i)=>({row:i+2,values:r})).filter(x=>x.values[0]===true);if(a.length!==1)throw new Error('Candidate Poolで再評価する候補を1件だけチェックしてください。');return a[0];}

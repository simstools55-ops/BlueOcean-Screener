# SIMS Blue Ocean Screener v0.14.10
...
v0.14.10 は、Claude回答の `checked_count` と実際の `serp_evidence` 件数に1件程度のずれがあっても、実証拠が最低5件あれば登録を継続できるよう入力耐性を改善しています。BOS自身が `serp_evidence.length` を実確認件数として正規化し、履歴にも正規化後の件数を保存します。SERPの到達帯・既存記事検出・5候補×最大5サイクルの運用仕様は変更ありません。

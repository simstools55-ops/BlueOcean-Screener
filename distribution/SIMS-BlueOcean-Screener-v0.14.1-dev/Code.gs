...
function sbosReevaluateSelectedPoolCandidate(){
  ...
  const html=HtmlService.createHtmlOutput('<!doctype html><html><head><base target="_top"><style>body{font-family:Arial;margin:0;color:#202124}.w{padding:18px}.t{font-size:20px;font-weight:700}.m{background:#e8f0fe;padding:10px;border-radius:8px;line-height:1.55;margin:10px 0}textarea{width:100%;box-sizing:border-box;border:1px solid #dadce0;border-radius:7px;padding:10px;font-family:monospace;font-size:12px}#req{height:220px}#ans{height:220px}.a{display:flex;justify-content:flex-end;gap:8px;margin-top:8px}button{border:0;border-radius:6px;padding:9px 14px;font-weight:600;cursor:pointer}.p{background:#1a73e8;color:#fff}.s{background:#f1f3f4}.st{background:#f8fafd;padding:8px;border-radius:6px;margin-top:7px;white-space:pre-wrap}</style></head><body><div class="w"><div class="t">保存済み候補を再評価</div><div class="m"><b>キーワード:</b> '+sbosEscapeHtml_(kw)+'<br><b>事前カニバリ:</b> '+sbosEscapeHtml_(risk.risk)+'<br>最新SERP上位30件で再評価します。</div><textarea id="req" readonly>'+sbosEscapeHtml_(req)+'</textarea><div class="a"><button id="cp" class="p" onclick="copyReq()">依頼文をコピー</button></div><div id="cst" class="st"></div><textarea id="ans" placeholder="Claude / Geminiの回答全文を貼り付け"></textarea><div id="st" class="st">回答待ち</div><div class="a"><button class="s" onclick="google.script.host.close()">閉じる</button><button id="go" class="p" onclick="reg()">再評価結果を登録</button></div><script>async function copyReq(){const t=document.getElementById("req"),b=document.getElementById("cp");try{await navigator.clipboard.writeText(t.value)}catch(e){t.select();document.execCommand("copy")}b.textContent="✓ コピーしました";document.getElementById("cst").textContent="✓ 依頼文をコピーしました。Claude / Geminiへ貼り付けてください。";setTimeout(()=>b.textContent="依頼文をコピー",2200)}function reg(){const t=document.getElementById("ans").value;if(!t.trim()){document.getElementById("st").textContent="回答全文を貼り付けてください。";return}const b=document.getElementById("go");b.disabled=true;b.textContent="登録中…";google.script.run.withSuccessHandler(r=>{b.disabled=false;b.textContent="登録完了";document.getElementById("st").textContent="再評価: "+r.grade+" / 推定到達順位 "+r.estimatedRank+"位\n"+(r.creatorEligible?"次はカニバリ精査後、aCreatorへ進めます。":"aCreator対象外です。")}).withFailureHandler(e=>{b.disabled=false;b.textContent="再評価結果を登録";document.getElementById("st").textContent="エラー: "+(e&&e.message?e.message:e)}).sbosRegisterPoolReevaluation(document.getElementById("ans").value)}</script></div></body></html>').setWidth(820).setHeight(720);SpreadsheetApp.getUi().showModalDialog(html,'9. 保存済み候補を再評価');
}

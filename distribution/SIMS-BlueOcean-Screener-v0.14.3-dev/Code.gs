...
function sbosAbout() {
  SpreadsheetApp.getUi().alert(
    SBOS_PRODUCT_NAME,
    'Version: ' + SBOS_VERSION + '\n\n3語・4語のロングテール候補を選別し、上位30件SERPによる4段階判定、カニバリ確認、候補プール、aCreator依頼文生成までを支援します。',
...
function sbosApplySerpReviewPayload_(payload, source) {
  ...
  const summary='SERP精査結果登録済み（GREEN '+counts.GREEN+' / YELLOW '+counts.YELLOW+' / PALE PINK '+counts.PALE_PINK+' / RED '+counts.RED+' / BLOCK '+counts.BLOCK+'）';
  sbosSetHomeStatus_(summary);
  sbosSyncCandidatePool_();
  sbosSaveCurrentBlogSession_();
  return {applied:applied,green:counts.GREEN,yellow:counts.YELLOW,palePink:counts.PALE_PINK,red:counts.RED,block:counts.BLOCK,clustered:clustered,requestId:String(payload.request_id||''),newSiteMode:sbosIsNewSiteMode_()};
}
...
function sbosQueueSelectedCreatorCases() {
  ...
  sbosShowWorkflowResult_(
    'aCreator依頼待ちへ登録しました',
    '<b>登録:</b> '+selected.length+'件<br><br>案件は保存されています。今日すべて処理する必要はありません。次回はCandidatesで「依頼待ち」の案件をチェックし、「7. GREEN / YELLOW / PALE PINK候補をaCreatorで処理」から続けられます。',
    '4. 候補・進捗を確認',
    'sbosOpenCandidates'
  );
}
...
function sbosEnsureLightweightHome_(force) {
  ...
  if (needsBuild) {
    ...
    const rows = [
      ...
      ['・未処理およびBLOCK以外の候補はCandidate Poolへ保存し、後から最新SERPで再評価できます。','','','','','','','','','']
    ];
    if (sbosIsNewSiteMode_()) {
      rows[1] = ['モード','','','対象サイト','','','','','',''];
      rows[5] = ['現在の候補状況','','','','','','','','',''];
...
function sbosSyncCandidatePool_(){
  ...
  let removed=0;
  if(pool.getLastRow()>=2){for(let rr=pool.getLastRow();rr>=2;rr--){const pv=pool.getRange(rr,1,1,12).getValues()[0];if(String(pv[10]||'')===siteKey&&sbosCandidatePoolIsGraduated_(pv[7],'')){pool.deleteRow(rr);removed++;}}}

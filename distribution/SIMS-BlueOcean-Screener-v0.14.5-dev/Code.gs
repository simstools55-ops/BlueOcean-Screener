  ...
  try {
    ...
    const payload = {
      ...
      evaluation_policy: isNewSite ? {
        ...
        require_top30_per_candidate: true,
  ...
  return '<!doctype html>' +
    ...
    'function registerSerp(){' +
      ...
      'google.script.run.withSuccessHandler(function(r){' +
        ...
        'if(newSiteMode){q("nextCandidates").style.display="inline-flex"}else if(r.green>0||r.yellow>0||(r.palePink||0)>0){q("nextCannibal").style.display="inline-flex"}else{q("nextCandidates").style.display="inline-flex"}' +
      '}).withFailureHandler(function(e){' +
...
function sbosBuildSerpPackageReadme_(p) {
  return [
    ...
    '各候補について自然検索の上位30件を実際に確認し、AIは色判定をせず estimated_reachable_rank を返します。',
...
function sbosBuildSerpReviewRequestMarkdown_(p) {
  ...
  const lines = [
    ...
    '- AIは各候補について現在の自然検索上位30件を1位〜30位まで実際に確認する',
    ...
    '## 上位30件の必須確認','',
    '- checked_count は30以上',
    '- serp_top30 は少なくとも30件',
    '- serp_top30[] は rank, title, url を必須とし、可能なら site_type, weakness_or_strength も返す',
    '- 企業・公式・大手メディアによる占有度',
    '- 個人ブログ、Q&A、フォーラム等の混在',
...
function sbosBuildGeminiSerpReviewMarkdown_(p) {
  return [
    ...
    'GeminiのWeb検索機能を使い、各候補について現在の自然検索上位30件を実際に確認してください。',
...
function sbosApplySerpReviewPayload_(payload, source) {
  ...
  data.forEach((row,i) => {
    ...
    const top30 = Array.isArray(r.serp_top30) ? r.serp_top30 : [];
    if (!r.hard_block && (!isFinite(checked) || checked < 30 || top30.length < 30)) throw new Error('上位30件SERP確認が不足しています: ' + r.main_keyword + ' / checked_count=' + r.checked_count + ' / serp_top30=' + top30.length);
  ...
  // SERP後の実運用候補はGREEN / YELLOW / PALE PINKだけにする。
  // RED / BLOCKは履歴・SERPアーカイブには残すが、Candidates / Candidate Poolからは除外する。
  const excluded = sbosPruneRejectedCandidates_(pendingAfter === 0);
  sbosApplyCandidateFormatting_();
  ...
  const summary='SERP精査結果登録済み（GREEN '+counts.GREEN+' / YELLOW '+counts.YELLOW+' / PALE PINK '+counts.PALE_PINK+' / 除外 '+(counts.RED+counts.BLOCK+clustered)+'）';
  sbosSetHomeStatus_(summary);
  sbosSyncCandidatePool_();
  sbosSaveCurrentBlogSession_();
  return {applied:applied,green:counts.GREEN,yellow:counts.YELLOW,palePink:counts.PALE_PINK,red:counts.RED,block:counts.BLOCK,clustered:clustered,excluded:excluded,requestId:String(payload.request_id||''),newSiteMode:sbosIsNewSiteMode_()};
}
...
function sbosBuildClaudeSerpPrompt_(p) {
  return [
    ...
    '',
    'Task: Review every candidate using real, current web search results. For EACH candidate, inspect the current top 30 organic SERP results.',
    ...
    'checked_count must be at least 30 and serp_top30 must contain at least 30 ranked entries for every non-blocked candidate.',
...
function sbosSerpResultJsonSchema_() {
  ...
  const result = {
    ...
    properties:{
      ...
      serp_top30:{type:'array', items:serpItem},
    ...
    required:['rank','main_keyword','checked_count','serp_top30','estimated_reachable_rank','confidence_percent','hard_block','block_reason','blue_ocean_score','search_intent','evidence_summary','exact_or_near_exact_competitors','big_site_pressure','intent_gap','personal_blog_chance','suggested_four_word_queries','sources'],
...
function sbosReevaluationRequestText_(keyword,requestId){return ['# SIMS BOS 保存済み候補 SERP再評価','', '対象キーワード: '+keyword,'request_id: '+requestId,'','現在のWeb検索でこのキーワードを検索し、自然検索の上位30件を1位から30位まで実際に確認してください。','AIはGREEN/YELLOW/PALE PINK/REDを決めません。現実的な estimated_reachable_rank を返してください。','独立した検索意図が成立しない、需要Signalが確認できない等の強制停止条件がある場合だけ hard_block=true としてください。','checked_countは30以上、serp_top30は30件以上必須です。','','回答末尾に SIMS_BOS_SERP_REVIEW_RESULT_V1 の完全JSONを ```json``` で出力してください。resultsはこの1件だけです。','必要フィールド: main_keyword, checked_count, serp_top30[{rank,title,url,site_type,weakness_or_strength}], estimated_reachable_rank, confidence_percent, hard_block, block_reason, blue_ocean_score, search_intent, evidence_summary, exact_or_near_exact_competitors, big_site_pressure, intent_gap, personal_blog_chance, suggested_four_word_queries, sources','', '判定基準はSIMS側で固定: 1-10 GREEN / 11-20 YELLOW / 21-30 PALE PINK / 31+ RED / hard_block=true はBLOCK'].join('\n');}

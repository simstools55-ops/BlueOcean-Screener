/**
 * Blue Ocean Screener v0.14.6
 * Single-Code Apps Script distribution.
 * v0.14.6: SERP review runs in 5-candidate x top-20 cycles; stop at 10 GREEN+YELLOW or 5 cycles.
 * v0.14.5: Gemini SERP review Markdown output added alongside Claude ZIP; SERP response import is AI-provider neutral.
 * v0.14.4: 2-word keywords join screening; top 30 advance to SERP; RED/BLOCK are excluded after review.
 * v0.14.3: standard menu cleanup, single-item aCreator referral, Candidate Pool graduation/wrap.
 * v0.14.2: Dialog action buttons show immediate busy/copy feedback with spinners while processing.
 * v0.14.1: Candidate Pool is displayed strictly per current target-site context; unassigned searches are isolated from named sites.
 * v0.14.0: rank-based four-tier SERP judgment + persistent candidate pool / re-evaluation.
 *
...
function sbosStartNewSiteDiscoveryMode() {
  ...
  sbosShowWorkflowResult_(
    ...
    '<b>判定:</b> 推定到達順位をBOSが GREEN / YELLOW / PALE PINK / RED に変換します。',
    '2. キーワードを読み込む',
    'sbosShowDrivePicker'
  );
...
function sbosAbout() {
  SpreadsheetApp.getUi().alert(
    SBOS_PRODUCT_NAME,
    'Version: ' + SBOS_VERSION + '\n\n2語・3語・4語候補を一次評価し、上位30件をSERP精査へ送り、GREEN / YELLOW / PALE PINKだけを記事候補として残します。',
...
const SBOS_SHEETS = {
  ...
  CANDIDATE_HISTORY: '_CandidateHistory'
};
...
function sbosRefreshHomeSummary_() {
  ...
  if (kw && kw.getLastRow() >= 2) {
    ...
    total=vals.length; two=vals.filter(r=>Number(r[5])===2).length; three=vals.filter(r=>Number(r[5])===3).length; existing4=vals.filter(r=>Number(r[5])===4).length;
  }
  let generated4=0,serpWait=0,cannibalWait=0,green=0,tryCount=0,yellow=0,palePink=0,red=0,block=0,clustered=0,creatorUnqueued=0,creatorQueued=0,creatorDone=0,sbmLinked=0;
  if (cand && cand.getLastRow() >= 2) {
    cand.getRange(2,1,cand.getLastRow()-1,18).getDisplayValues().forEach(r=>{
      ...
      if(['GREEN','YELLOW','PALE_PINK','TRY'].includes(st)){
        if(st==='GREEN') green++; else if(st==='YELLOW') yellow++; else if(st==='PALE_PINK') palePink++; else tryCount++;
  ...
  home.getRange('B9').setValue(green); home.getRange('D9').setValue(yellow); home.getRange('F9').setValue(palePink); home.getRange('H9').setValue(red);
  const poolCount=sbosCandidatePoolCountForCurrentContext_();
  home.getRange('A22').setValue('・Candidate Pool（'+sbosCandidatePoolContextLabel_()+'）：'+poolCount+'件。別サイトの候補は表示・再評価対象に含めません。');
  if (sbosIsNewSiteMode_()) {
    home.getRange('A9').setValue('GREEN');
    home.getRange('C9').setValue('YELLOW');
    home.getRange('E9').setValue('PALE PINK');
    home.getRange('G9').setValue('RED');
    home.getRange('D9').setValue(yellow);
    home.getRange('F9').setValue(palePink);
    home.getRange('H9').setValue(red);
    ...
    home.getRange('A21').setValue('・最終色は推定到達順位でBOSが判定します。AIは上位30件SERPの証拠収集と順位推定を担当します。');
...
function sbosBuildMenu_() {
  ...
  ui.createMenu('Blue Ocean Screener')
    ...
    .addItem('4. 候補・進捗を確認する', 'sbosOpenCandidates')
    .addItem('5. SERP精査を進める', 'sbosShowSerpWorkflowDialog')
...
function sbosSessionCounts_() {
  ...
  if (sh && sh.getLastRow() >= 2) {
    ...
    vals.forEach(r => {
      const finalSt=sbosStatusCode_(r[1]);
      if (!['GREEN','YELLOW','PALE_PINK','TRY'].includes(finalSt)) return;
      if(finalSt==='GREEN') green++;
...
function sbosSaveCurrentBlogSession_() {
  try { sbosSyncCandidatePool_(); } catch(e) { console.error(e); }
...
function sbosShowCannibalEvidencePicker() {
  ...
  const eligible = vals.filter(r => ['GREEN','YELLOW','PALE_PINK'].includes(String(r[12]||'').toUpperCase()) && sbosStatusCode_(r[1]) === 'CANNIBAL_PENDING');
  if (!eligible.length) {
    SpreadsheetApp.getUi().alert('カニバリ精査対象がありません', 'GREEN / YELLOW / PALE PINKのカニバリ精査待ち候補がありません。', SpreadsheetApp.getUi().ButtonSet.OK);
    return;
  }
  const template = HtmlService.createTemplateFromFile('DrivePicker');
  template.currentSiteName = sbosGetSetting_('site_name') || '';
...
function sbosParseKeywordText_(text, filename) {
  ...
  const meta = {
    ...
    total: rows.length,
    two: rows.filter(x => countWords(x) === 2).length,
...
function sbosMarkPrimaryCandidates_() {
  ...
  data.forEach((r,i) => {
    ...
    const eligible = words === 2 || words === 3 || words === 4;
...
function sbosShowScreeningResult_(meta) {
  ...
  if (count > 0) {
    sbosShowWorkflowResult_(
      '一次選抜完了',
      '<b>SERP精査対象（一次評価 上位30件まで）:</b> ' + count + '件<br>' +
      '<b>新規4語深掘り候補:</b> ' + Number(meta.generated4 || 0) + '件<br><br>' +
      '2語・3語・4語を同一基準で一次評価し、上位30件までをSERP精査へ送ります。最終的にGREEN / YELLOW / PALE PINKだけを残します。',
      '5. SERP精査へ', 'sbosShowSerpWorkflowDialog'
    );
    return;
  }
...
function sbosStartScreeningFromDialog() {
  ...
  sbosSetState_('serp_completed_cycles', 0);
  sbosSetState_('serp_last_completed_request_id', '');
  sbosSetState_('serp_request_id', '');
  sbosSetState_('serp_active_batch_keys', '');
...
function sbosRunScreening_() {
  ...
  // 入力ファイルに実在する2語・3語・4語候補を同一の一次評価へ載せる。
  // 一次段階では固定スコア閾値で大量に捨てず、全候補を順位付けして上位30件までSERP精査へ送る。
  rows.forEach(r => {
    const words = Number(r[5]);
    if (![2,3,4].includes(words) || r[12] !== 'YES') return;
    ...
    const source = words === 2 ? 'EXISTING_2WORD' : (words === 4 ? 'EXISTING_4WORD' : 'EXISTING_3WORD');
    out.push({
  ...
  // 一次選抜はPre Score順の上位30件まで。語数別の枠は設けない。
...
function sbosWriteCandidates_(items) {
  const sh = SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATES);
  if (sh.getLastRow() > 1) sh.getRange(2,1,sh.getLastRow()-1,30).clearContent();
  ...
  if (vals.length) sh.getRange(2,1,vals.length,30).setValues(vals);
  sbosApplyCandidateFormatting_();
}
...
function sbosApplyCandidateFormatting_() {
  ...
  sh.getRange(1,1).setNote('処理する候補にチェックします。aCreator依頼は1回1件です。');
  ...
  const last = sh.getLastRow();
  if (last >= 2) {
    const vals = sh.getRange(2,1,last-1,30).getDisplayValues();
    vals.forEach((r,i) => {
      const row=i+2, st=sbosStatusCode_(r[1]), creator=String(r[10]||''), articleId=String(r[13]||'');
      if (creator === 'SIMS Manager登録済み' || articleId) {
        sh.getRange(row,1,1,30).setBackground('#eeeeee').setFontColor('#777777');
        return;
      }
      sh.getRange(row,1,1,30).setFontColor('#202124').setBackground(null);
      let bg='#f1f3f4';
      if (st === 'CANNIBAL_PENDING' || st === 'YELLOW') bg='#fff2cc';
      if (st === 'PALE_PINK') bg='#fce8ef';
      if (st === 'RED') bg='#f4cccc';
  ...
  if (!sh.getFilter() && sh.getLastRow() >= 1) {
    try { sh.getRange(1,1,Math.max(1,sh.getLastRow()),30).createFilter(); } catch(e) {}
  }
...
function sbosCandidateSortPriority_(row) {
  ...
  if (['GREEN','YELLOW','PALE_PINK','TRY'].includes(st) && creator === '依頼待ち') return 10;
  if (['GREEN','YELLOW','PALE_PINK','TRY'].includes(st) && creator === '作成済み') return 20;
  if (st === 'GREEN') return 5;
  if (st === 'YELLOW') return 6;
  if (st === 'PALE_PINK') return 7;
  if (st === 'TRY') return 8;
...
function sbosSortCandidatesByStatus_() {
  ...
  const vals = sh.getRange(2,1,n,30).getValues();
  vals.sort((a,b) => {
  ...
  sh.getRange(2,1,n,30).setValues(vals);
...
function sbosCreateSerpReviewPackage(options) {
  ...
  const provider = String(options.provider || 'CLAUDE').toUpperCase();
  const useGeminiFile = provider === 'GEMINI';
  const ui = SpreadsheetApp.getUi();
  ...
  try {
    ...
    const progressBefore = sbosGetSerpCycleProgress_();
    if (progressBefore.stopReached) throw new Error(progressBefore.stopReason || 'SERP精査の停止条件に到達しています。');
    const requestedRows = [], pendingRows = [];
    values.forEach((r,idx) => {
      const st = sbosStatusCode_(r[12]);
      const item = {rowIndex:idx,row:r};
      if (r[12] === 'REQUESTED') requestedRows.push(item);
      else if (st === 'PENDING') pendingRows.push(item);
    });
    const selectedRows = (requestedRows.length ? requestedRows : pendingRows).slice(0, SBOS_THRESHOLDS.SERP_BATCH_SIZE);
    const candidates = selectedRows.map(x => {
      const r=x.row;
      return {rank:Number(r[17])||0,main_keyword:r[2],words:Number(r[3])||0,pre_score:Number(r[4])||0,search_intent:r[7],source:r[9],intent_key:r[11],source_metrics:r[8]||''};
    });
    ...
    const payload = {
      ...
      cycle: {number: progressBefore.completedCycles + 1, batch_size: SBOS_THRESHOLDS.SERP_BATCH_SIZE, serp_depth: SBOS_THRESHOLDS.SERP_CHECK_DEPTH, max_cycles: SBOS_THRESHOLDS.SERP_MAX_CYCLES, target_green_yellow: SBOS_THRESHOLDS.SERP_TARGET_YELLOW_PLUS},
      evaluation_policy: isNewSite ? {
        ...
        require_top20_per_candidate: true,
        final_grade_owner: 'SIMS_BOS',
        grade_mapping: '1-10 GREEN / 11-20 YELLOW / 21-30 PALE_PINK / 31+ RED / hard_block=true => BLOCK',
        ...
        purpose: '個人ブログが上位表示を狙える2語・3語・4語のBlue Ocean候補を実SERPで精査する',
        do_not_assume_volume_zero_means_no_demand: true,
        require_web_verification: true,
        require_top20_per_candidate: true,
        final_grade_owner: 'SIMS_BOS',
        grade_mapping: '1-10 GREEN / 11-20 YELLOW / 21-30 PALE_PINK / 31+ RED / hard_block=true => BLOCK',
        do_not_finalize_article_before_cannibalization_check: true,
        keyword_planner_metrics_policy: '月間検索数・3か月推移・前年比は需要Signalとして利用する。Google Ads競合性はSEO難易度と同一視せず、広告主需要の補助情報としてのみ扱う'
      },
      candidates: candidates
    };
    ...
    if (useGeminiFile) {
      stage = 'Gemini用依頼ファイル生成';
      const tFile = new Date().getTime();
      outputName = 'SIMS-BOS-' + safeSite + '-Gemini-SERP-Review-' + stamp + '.md';
      outputBlob = Utilities.newBlob(sbosBuildGeminiSerpReviewMarkdown_(payload), 'text/markdown', outputName);
      timing.fileBuildMs = new Date().getTime() - tFile;
    } else {
      stage = 'Claude用ZIP生成';
      ...
      outputName = 'SIMS-BOS-' + safeSite + '-Claude-SERP-Review-' + stamp + '.zip';
      outputBlob = Utilities.zip(blobs, outputName);
    ...
    const file = folder.createFile(outputBlob);
    timing.driveSaveMs = new Date().getTime() - tDrive;

    stage = '候補状態更新';
    const tStatus = new Date().getTime();
    const selectedIndexSet = new Set(selectedRows.map(x => x.rowIndex));
    const statusValues = values.map((r,idx) => [selectedIndexSet.has(idx) && sbosStatusCode_(r[12]) === 'PENDING' ? 'REQUESTED' : r[12]]);
    ...
    sbosSetState_('serp_active_batch_keys', JSON.stringify(candidates.map(c => sbosKeywordMatchKey_(c.main_keyword)).filter(Boolean)));
    sbosSetState_('serp_package_file_id', file.getId());
    sbosSetState_('serp_package_file_name', outputName);
    sbosSetState_('serp_request_provider', provider);
    sbosSetHomeStatus_('SERP精査依頼作成済み');
    ...
    if (!suppressDialog) {
      sbosShowWorkflowResult_(
        ...
        '<b>ファイル名:</b> ' + sbosEscapeHtml_(outputName) + '<br>' +
        '<b>保存先:</b> ' + sbosEscapeHtml_(folderName || folder.getName() || 'マイドライブ') + '<br>' +
        '<b>処理時間:</b> ' + (timing.totalMs / 1000).toFixed(1) + '秒<br><br>' +
        (useGeminiFile ? 'このMarkdownファイルをGeminiへアップロードしてSERP精査を依頼してください。' : 'このZIPをClaude.aiへアップロードしてSERP精査を依頼してください。'),
        '',
        ''
      );
    }
    return {
      ...
      cycle:progressBefore.completedCycles + 1,
      maxCycles:SBOS_THRESHOLDS.SERP_MAX_CYCLES,
      yellowPlusTotal:progressBefore.yellowPlusTotal,
      yellowPlusTarget:SBOS_THRESHOLDS.SERP_TARGET_YELLOW_PLUS,
      fileName:outputName,
      provider:provider,
...
function sbosCreateGeminiSerpReviewFileForWorkflow() {
  const r = sbosCreateSerpReviewPackage({suppressDialog:true, provider:'GEMINI'});
  if (!r) throw new Error('Gemini用SERP精査依頼ファイルを作成できませんでした。');
  return r;
}

function sbosGetSerpCycleProgress_() {
  const completedCycles = Number(sbosGetState_('serp_completed_cycles') || 0);
  const sh = SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATES);
  let green=0,yellow=0,palePink=0,pending=0,requested=0;
  if (sh && sh.getLastRow() >= 2) {
    sh.getRange(2,1,sh.getLastRow()-1,13).getDisplayValues().forEach(r => {
      const st=sbosStatusCode_(r[12]);
      if(st==='GREEN')green++; else if(st==='YELLOW')yellow++; else if(st==='PALE_PINK')palePink++; else if(st==='PENDING')pending++; else if(r[12]==='REQUESTED')requested++;
    });
  }
  const yellowPlusTotal=green+yellow;
  let stopReason='';
  if(yellowPlusTotal>=SBOS_THRESHOLDS.SERP_TARGET_YELLOW_PLUS) stopReason='GREEN＋YELLOWが目標'+SBOS_THRESHOLDS.SERP_TARGET_YELLOW_PLUS+'件に到達しました。';
  else if(completedCycles>=SBOS_THRESHOLDS.SERP_MAX_CYCLES) stopReason='SERP精査を最大'+SBOS_THRESHOLDS.SERP_MAX_CYCLES+'サイクル完了しました。';
  else if((pending+requested)===0) stopReason='SERP未精査候補がありません。';
  return {completedCycles,maxCycles:SBOS_THRESHOLDS.SERP_MAX_CYCLES,green,yellow,palePink,yellowPlusTotal,target:SBOS_THRESHOLDS.SERP_TARGET_YELLOW_PLUS,pending,requested,stopReached:!!stopReason,stopReason};
}
...
function sbosShowSerpWorkflowDialog() {
  ...
  SpreadsheetApp.getUi().showModalDialog(html, '5. SERP精査');
}
...
function sbosBuildSerpWorkflowHtml_(isNewSite, siteName, pending) {
  ...
  return '<!doctype html>' +
    ...
    '1サイクルは最大5候補 × 自然検索上位20件です。停止条件はGREEN＋YELLOW 10件、または5サイクル完了です。<br>' +
    'Claude用ZIPまたはGemini用Markdownを作成し、回答登録までこの画面で続けて処理できます。</div>' +

    '<div class="b"><div class="l">① SERP精査依頼を作成</div>' +
    '<div id="pkg" class="st">未作成</div>' +
    '<div class="a">' +
    '<button id="serpClaudeBtn" type="button" class="s"><span class="sp"></span><span class="tx">Claude用ZIPを作成</span></button>' +
    '<button id="serpGeminiBtn" type="button" class="p"><span class="sp"></span><span class="tx">Gemini用ファイルを作成</span></button>' +
    '</div></div>' +

    '<div class="b"><div class="l">② AI回答全文を貼り付け</div>' +
    '<textarea id="ans" placeholder="Claude / Gemini の回答全文をここへ貼り付け"></textarea>' +
    '<div id="rst" class="st">回答待ち</div><div class="a">' +
    '<button id="closeBtn" type="button" class="s">閉じる</button>' +
    '<button id="nextCycle" type="button" class="p" style="display:none"><span class="sp"></span><span class="tx">次の5候補をSERP精査</span></button>' +
    ...
    'function createPackage(provider){' +
      'var gemini=provider==="GEMINI",b=q(gemini?"serpGeminiBtn":"serpClaudeBtn"),s=q("pkg");' +
      'busy(b,true,gemini?"Gemini用作成中…":"Claude用作成中…");' +
      's.textContent=(gemini?"Gemini用Markdown":"Claude用ZIP")+"を作成しています…";' +
      'showOv(gemini?"候補を確認してGemini用Markdownを準備しています。\\nGoogle Driveへの保存完了までお待ちください。":"候補を確認してClaude用ZIPを準備しています。\\nGoogle Driveへの保存完了までお待ちください。");' +
      'setTimeout(function(){' +
        'var runner=google.script.run.withSuccessHandler(function(r){' +
          'hideOv();busy(b,false);' +
          's.textContent="作成完了\\n形式: "+(gemini?"Gemini用Markdown":"Claude用ZIP")+"\\nファイル: "+r.fileName+"\\n保存先: "+r.folderName+"\\n候補: "+r.count+"件\\n処理時間: "+((r.elapsedMs||0)/1000).toFixed(1)+"秒\\n\\n"+(gemini?"このファイルをGeminiへアップロードしてください。":"このZIPをClaude.aiへアップロードしてください。");' +
        '}).withFailureHandler(function(e){' +
          'hideOv();busy(b,false);' +
          's.textContent="エラー\\n"+(e&&e.message?e.message:String(e));' +
        '});' +
        'if(gemini){runner.sbosCreateGeminiSerpReviewFileForWorkflow()}else{runner.sbosCreateSerpReviewPackageForWorkflow()}' +
      '},150);' +
    '}' +

    'function registerSerp(){' +
      'var b=q("regSerp"),ans=q("ans").value,s=q("rst");' +
      'if(!ans.trim()){s.textContent="Claude / Gemini の回答全文を貼り付けてください。";return}' +
      'busy(b,true,"処理中…");s.textContent="回答を検証・登録しています…";' +
      'google.script.run.withSuccessHandler(function(r){' +
        'busy(b,false);' +
        's.textContent="登録完了\\n反映: "+r.applied+"件\\nGREEN: "+r.green+" / YELLOW: "+r.yellow+" / BLOCK: "+r.block+(r.newSiteMode?"\\n新規サイト適性評価を反映済み":"");' +
        'b.style.display="none";' +
        'q("serpClaudeBtn").style.display="none";q("serpGeminiBtn").style.display="none";if(r.shouldContinue){q("nextCycle").style.display="inline-flex"}else if(newSiteMode){q("nextCandidates").style.display="inline-flex"}else if(r.yellowPlusTotal>0||(r.palePinkTotal||0)>0){q("nextCannibal").style.display="inline-flex"}else{q("nextCandidates").style.display="inline-flex"}' +
      '}).withFailureHandler(function(e){' +
    ...
    'function bind(){' +
      'q("serpClaudeBtn").addEventListener("click",function(){createPackage("CLAUDE")});q("serpGeminiBtn").addEventListener("click",function(){createPackage("GEMINI")});' +
...
function sbosBuildSerpPackageReadme_(p) {
  return [
    ...
    '- Mode: ' + (p.mode || SBOS_MODE.EXISTING_SITE),
    '- Site: ' + ((p.site && p.site.name) || '指定なし'),
    '- Candidates: ' + p.candidate_count,'',
    '## 利用方法','',
    'このZIPをClaudeへそのままアップロードし、Web検索で全候補を検証してください。',
    '各候補について自然検索の上位20件を実際に確認し、AIは色判定をせず estimated_reachable_rank を返します。',
    '最終色はBOSが固定基準で判定します: 1〜10位 GREEN / 11〜20位 YELLOW / 21〜30位 PALE PINK / 31位以下 RED。',
...
function sbosBuildSerpReviewRequestMarkdown_(p) {
  const isNew = p && p.mode === SBOS_MODE.NEW_SITE;
  const lines = [
    ...
    '- mode: ' + (p.mode || SBOS_MODE.EXISTING_SITE),
    '- site: ' + ((p.site && p.site.name) || '指定なし'),
    '- site_url: ' + ((p.site && p.site.url) || ''),
    '- candidates: ' + p.candidate_count,'',
    '## 重要な役割分担','',
    '- AIは GREEN / YELLOW / PALE PINK / RED を決めない',
    '- AIは各候補について現在の自然検索上位20件を1位〜20位まで実際に確認する',
    '- AIは証拠を基に estimated_reachable_rank を返す',
    '- BOSが estimated_reachable_rank から最終色を機械判定する',
    '- 1〜10位: GREEN / 11〜20位: YELLOW / 21〜30位: PALE PINK / 31位以下: RED',
    '- 独立検索意図が成立しない、需要Signalが確認できない等の強制停止条件だけ hard_block=true とする','',
    '## 上位20件の必須確認','',
    '- checked_count は20以上',
    '- serp_top20 は少なくとも20件',
    '- serp_top20[] は rank, title, url を必須とし、可能なら site_type, weakness_or_strength も返す',
    '- 企業・公式・大手メディアによる占有度',
    '- 個人ブログ、Q&A、フォーラム等の混在',
    '- 検索意図の一致・ズレ、情報鮮度、弱い結果の有無',
    '- source=GENERATED_4WORD は実SERP・サジェスト・Q&A等で需要Signalを確認する',''
  ];
  if (isNew) {
    lines.push('## 新規サイト向け追加評価','',
      '対象サイトや既存記事を前提にせず、新規サイトの入口テーマとしての適性も評価してください。',
      '- new_site_fit_score (0-100)',
      '- new_site_dimensions {entry_ease,demand,serp_gap,expansion,cluster_potential,continuity,risk}',
      '- new_site_assessment',
      'これらは補助評価です。最終色自体は estimated_reachable_rank をBOSが4段階へ変換します。','');
  } else {
    lines.push('## 対象サイトとの関係','',
      '対象サイトで十分に作り込んだ新記事を公開した場合の現実的な到達順位を推定してください。',
      'SERP精査段階では既存記事との最終カニバリ判定を行わず、GREEN / YELLOW / PALE PINKは後段のカニバリ精査へ送ります。','');
  }
  lines.push('## 返却JSON','',
    '回答末尾に `SIMS_BOS_SERP_REVIEW_RESULT_V1` の完全JSONを ```json コードブロックでインライン出力してください。',
    'JSONを別ファイルだけに格納して終わらせないでください。','',
    'results[] 必須フィールド:',
    '- rank',
    '- main_keyword',
    '- checked_count',
    '- serp_top20[]',
    '- estimated_reachable_rank',
    '- confidence_percent',
    '- hard_block',
    '- block_reason',
  ...
  p.candidates.forEach(c => {
    ...
    if (c.intent_key) lines.push('- intent_key: ' + c.intent_key);
...
function sbosBuildGeminiSerpReviewMarkdown_(p) {
  return [
    '# SIMS Blue Ocean Screener - Gemini SERP精査依頼',
    '',
    'このMarkdownファイル全体がSERP精査依頼です。ZIP展開は不要です。',
    'GeminiのWeb検索機能を使い、各候補について現在の自然検索上位20件を実際に確認してください。',
    '1回の依頼は最大5候補です。各候補のserp_top20に1位〜20位の20要素を省略せず格納してください。',
    'checked_count=20と自己申告するだけでは不可です。serp_top20の実要素数も20件必要です。架空URL・未確認結果の補完は禁止です。',
    '推測だけで順位を決めず、検索結果の証拠を基に estimated_reachable_rank を返してください。',
    'GREEN / YELLOW / PALE PINK / RED の色判定は行わず、最終色はBOSが機械判定します。',
    '回答末尾に SIMS_BOS_SERP_REVIEW_RESULT_V1 の完全JSONを必ずインライン出力してください。',
    '',
    '---',
    '',
    sbosBuildSerpReviewRequestMarkdown_(p)
  ].join('\n');
}
...
function sbosExtractContractJsonFromText_(rawText, expectedFormat) {
  ...
  if (parsed.length) {
    ...
    throw new Error(
      ...
      'AI回答末尾に format="' + expectedFormat + '" を含む完全JSONを出力してください。'
    );
  }

  // No valid JSON at all.
  throw new Error(
    expectedFormat + ' のJSONをAI回答全文から見つけられませんでした。\n' +
...
function sbosShowResultPasteDialog_(kind) {
  ...
  const title = isSerp ? '5. SERP精査結果を貼り付け・登録する' : '6. カニバリ精査結果を貼り付け・登録する';
  ...
  const nextLabel = isSerp ? '6. カニバリ精査へ' : '4. 候補・進捗を確認';
  const guide = isSerp
    ? 'SERP精査を行ったAI（Claude / Gemini）の回答を、説明文を含めて全文そのまま貼り付けてください。JSON部分だけを抜き出す必要はありません。'
  ...
  const html = HtmlService.createHtmlOutput(
    ...
    '<script>function work(b,on,label){if(!b)return;var t=b.querySelector(".tx");if(on){b.dataset.o=t?t.textContent:"";if(t)t.textContent=label||"処理中…";b.classList.add("working");b.disabled=true}else{if(t)t.textContent=b.dataset.o||"実行";b.classList.remove("working");b.disabled=false}}function submitAnswer(b){var t=document.getElementById("answer").value;if(!t.trim()){document.getElementById("msg").textContent="AIの回答全文を貼り付けてください。";return;}work(b,true,"登録中…");document.getElementById("msg").textContent="検証・登録中…";google.script.run.withSuccessHandler(function(r){work(b,false);document.getElementById("msg").textContent="登録完了\\n反映: "+r.applied+"件\\nGREEN: "+r.green+" / YELLOW: "+r.yellow+" / BLOCK: "+r.block+(r.clustered!==undefined?"\\nIntent統合: "+r.clustered+"件":"");document.getElementById("go").style.display="none";var n=document.getElementById("next");if(' + (isSerp ? '(r.green>0||r.yellow>0||(r.palePink||0)>0)' : 'true') + '){n.style.display="inline-flex";}else{var tx=n.querySelector(".tx");if(tx)tx.textContent="4. 候補を確認";n.style.display="inline-flex";}}).withFailureHandler(function(e){work(b,false);document.getElementById("msg").textContent="エラー: "+(e&&e.message?e.message:e);}).' + serverFn + '(t);}function nextStep(b){work(b,true,"移動中…");google.script.run.withSuccessHandler(function(){google.script.host.close();}).withFailureHandler(function(e){work(b,false);document.getElementById("msg").textContent="エラー: "+(e&&e.message?e.message:e);}).' + nextFn + '();}</script>' +
    '</body></html>'
...
function sbosApplySerpReviewPayload_(payload, source) {
  source = source || {};
  if (!payload || payload.format !== 'SIMS_BOS_SERP_REVIEW_RESULT_V1') throw new Error('SIMS_BOS_SERP_REVIEW_RESULT_V1 形式のSERP結果ではありません。');
  ...
  if (expectedRequestId && String(payload.request_id || '') !== expectedRequestId) throw new Error('request_idが一致しません。\n期待値: ' + expectedRequestId + '\n受信値: ' + String(payload.request_id || '未設定'));

  const sh = SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATES);
  if (!sh || sh.getLastRow() < 2) throw new Error('CandidatesシートにSERP候補がありません。');
  const data = sh.getRange(2,1,sh.getLastRow()-1,30).getValues();
  let requestedKeys=[];
  try { requestedKeys=JSON.parse(sbosGetState_('serp_active_batch_keys') || '[]'); } catch(e) { requestedKeys=[]; }
  if (!Array.isArray(requestedKeys) || !requestedKeys.length) requestedKeys = data.filter(row => row[12] === 'REQUESTED').slice(0,SBOS_THRESHOLDS.SERP_BATCH_SIZE).map(row => sbosKeywordMatchKey_(row[2])).filter(Boolean);
  if (!requestedKeys.length) throw new Error('現在のSERPサイクル候補がありません。先にSERP精査依頼を作成してください。');
  const resultMap = new Map();
  payload.results.forEach(r => {
    const key = sbosKeywordMatchKey_(r.main_keyword || '');
    if (!key) return;
    if (resultMap.has(key)) throw new Error('SERP結果に同一main_keywordが重複しています: ' + String(r.main_keyword || ''));
    resultMap.set(key, r);
  });

  requestedKeys.forEach(key => { if (!resultMap.has(key)) throw new Error('SERP回答に現在サイクルの候補が不足しています: ' + key); });
  if (resultMap.size !== requestedKeys.length) throw new Error('SERP回答の候補数が現在サイクルと一致しません。期待: '+requestedKeys.length+'件 / 受信: '+resultMap.size+'件');

  let applied = 0;
  const counts = {GREEN:0,YELLOW:0,PALE_PINK:0,RED:0,BLOCK:0};
  data.forEach((row,i) => {
    const key = sbosKeywordMatchKey_(row[2]);
    const r = resultMap.get(key);
    if (!r) return;
    const checked = Number(r.checked_count);
    const top20 = Array.isArray(r.serp_top20) ? r.serp_top20 : (Array.isArray(r.serp_top30) ? r.serp_top30 : []); // v0.14.5互換
    if (!isFinite(checked) || checked < SBOS_THRESHOLDS.SERP_CHECK_DEPTH || top20.length < SBOS_THRESHOLDS.SERP_CHECK_DEPTH) throw new Error('上位20件SERP確認が不足しています: ' + r.main_keyword + ' / checked_count=' + r.checked_count + ' / serp_top20=' + top20.length);
    const est = Number(r.estimated_reachable_rank);
    if (!isFinite(est) || est < 1) throw new Error('estimated_reachable_rank が不正です: ' + r.main_keyword);
    ...
    const finalDecision = r.hard_block === true ? 'BLOCK' : sbosSerpGradeFromRank_(est);
    counts[finalDecision]++;
    row[5] = Math.round(score);
    row[6] = sbosIsNewSiteMode_() ? 'NOT_RUN' : (['GREEN','YELLOW','PALE_PINK'].includes(finalDecision) ? 'PENDING' : 'NOT_RUN');
    row[7] = String(r.search_intent || row[7] || '');
    row[8] = String(r.evidence_summary || '');
    row[11] = sbosIntentKey_(sbosNormalizeKeyword_(row[2]));
    row[12] = finalDecision;
    row[1] = sbosIsNewSiteMode_() ? finalDecision : (['GREEN','YELLOW','PALE_PINK'].includes(finalDecision) ? sbosStatusLabel_('CANNIBAL_PENDING') : finalDecision);
    row[27] = Math.round(est);
    row[28] = String(payload.reviewed_at || sbosNow_());
    row[29] = finalDecision === 'BLOCK' ? String(r.block_reason || '強制停止条件') : '';

    if (sbosIsNewSiteMode_()) {
      const ns=Number(r.new_site_fit_score), dims=r.new_site_dimensions||{};
      if (isFinite(ns)) {
        sh.getRange(i+2,19,1,9).setValues([[Math.round(ns),Number(dims.entry_ease)||'',Number(dims.demand)||'',Number(dims.serp_gap)||'',Number(dims.expansion)||'',Number(dims.cluster_potential)||'',Number(dims.continuity)||'',Number(dims.risk)||'',String(r.new_site_assessment||'')]]);
      }
    }
    sbosAppendCandidateHistory_(row[2], finalDecision, est, payload.reviewed_at || sbosNow_(), 'SERP_REVIEW', String(r.evidence_summary || ''));
    applied++;
  });
  if (!applied) throw new Error('Candidatesのキーワードと一致するSERP結果がありませんでした。');
  sh.getRange(2,1,data.length,30).setValues(data);

  const sourceName = source.sourceName || 'Claude Manual Review';
  sbosWriteSerpReviewArchive_(payload, sourceName);
  const lastCompletedRequest = sbosGetState_('serp_last_completed_request_id') || '';
  const thisRequestId = String(payload.request_id || '');
  if (thisRequestId && thisRequestId !== lastCompletedRequest) {
    sbosSetState_('serp_completed_cycles', Number(sbosGetState_('serp_completed_cycles') || 0) + 1);
    sbosSetState_('serp_last_completed_request_id', thisRequestId);
    sbosSetState_('serp_active_batch_keys', '');
  }
  const excluded = sbosPruneRejectedCandidates_(false);
  const progress = sbosGetSerpCycleProgress_();
  const shouldContinue = !progress.stopReached && (progress.pending + progress.requested) > 0;
  const clustered = (progress.pending + progress.requested) === 0 ? sbosCollapseCandidateIntentDuplicates_() : 0;
  sbosApplyCandidateFormatting_();
  sbosSetState_('status', shouldContinue ? SBOS_STATUS.SERP_RUNNING : SBOS_STATUS.SERP_REVIEW_IMPORTED);
  ...
  const summary='SERP精査 '+progress.completedCycles+'/'+progress.maxCycles+'（GREEN＋YELLOW '+progress.yellowPlusTotal+'/'+progress.target+' / PALE PINK '+progress.palePink+'）'+(progress.stopReason?' / '+progress.stopReason:'');
  sbosSetHomeStatus_(summary);
  sbosSyncCandidatePool_();
  sbosSaveCurrentBlogSession_();
  return {applied:applied,green:counts.GREEN,yellow:counts.YELLOW,palePink:counts.PALE_PINK,red:counts.RED,block:counts.BLOCK,clustered:clustered,excluded:excluded,requestId:thisRequestId,newSiteMode:sbosIsNewSiteMode_(),completedCycles:progress.completedCycles,maxCycles:progress.maxCycles,yellowPlusTotal:progress.yellowPlusTotal,yellowPlusTarget:progress.target,palePinkTotal:progress.palePink,shouldContinue:shouldContinue,stopReason:progress.stopReason,pendingRemaining:progress.pending};
}

function sbosPruneRejectedCandidates_(removeClustered) {
  const sh = SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATES);
  ...
  let removed = 0;
  data.forEach(row => {
    const st = sbosStatusCode_(row[12] || row[1]);
    const reject = st === 'RED' || st === 'BLOCK' || (removeClustered === true && st === 'CLUSTERED');
    if (reject) removed++;
    else kept.push(row);
  });
  if (sh.getLastRow() > 1) sh.getRange(2,1,sh.getLastRow()-1,30).clearContent();
  if (kept.length) sh.getRange(2,1,kept.length,30).setValues(kept);
  return removed;
}
...
function sbosWriteSerpReviewArchive_(payload, filename) {
  ...
  const headers = [['Request ID','Reviewed At','Rank','Main Keyword','BOS Grade','Estimated Reachable Rank','Checked Count','Blue Ocean Score','Confidence','Hard Block','Block Reason','Exact/Near Exact Competitors','Big Site Pressure','Intent Gap','Personal Blog Chance','Suggested 4-Word Queries','Sources','Evidence Summary','Imported File']];
  if (sh.getLastRow() === 0 || String(sh.getRange('A1').getValue()) !== 'Request ID') { sh.clear(); sh.getRange(1,1,1,headers[0].length).setValues(headers).setFontWeight('bold'); }
  const vals = payload.results.map(r => [payload.request_id||'',payload.reviewed_at||'',r.rank||'',r.main_keyword||'',r.hard_block===true?'BLOCK':sbosSerpGradeFromRank_(r.estimated_reachable_rank),r.estimated_reachable_rank||'',r.checked_count||'',r.blue_ocean_score||'',r.confidence_percent||'',r.hard_block===true?'TRUE':'FALSE',r.block_reason||'',r.exact_or_near_exact_competitors||0,r.big_site_pressure||'',r.intent_gap||'',r.personal_blog_chance||'',(r.suggested_four_word_queries||[]).join(' | '),(r.sources||[]).join(' | '),r.evidence_summary||'',filename||'']);
  if (vals.length) sh.getRange(sh.getLastRow()+1,1,vals.length,headers[0].length).setValues(vals);
  sh.setFrozenRows(1); if (!sh.isSheetHidden()) sh.hideSheet();
}
...
function sbosBuildSerpReviewPayload_(requestId, candidates, siteName, siteUrl) {
  return {
    ...
    candidate_count:candidates.length,
    evaluation_policy:{
      purpose:'個人ブログが上位表示を狙える2語・3語・4語のBlue Ocean候補をWeb検索で精査する',
      do_not_assume_volume_zero_means_no_demand:true,
      require_web_verification:true,
...
function sbosSerpGradeFromRank_(rank) {
  const n = Number(rank);
  if (!isFinite(n) || n < 1) return 'RED';
  if (n <= 10) return 'GREEN';
  if (n <= 20) return 'YELLOW';
  if (n <= 30) return 'PALE_PINK';
  return 'RED';
}
...
function sbosBuildClaudeSerpPrompt_(p) {
  return [
    ...
    '',
    'Task: Review every candidate using real, current web search results. For EACH candidate, inspect the current top 20 organic SERP results.',
    'Do not decide GREEN/YELLOW/PALE_PINK/RED yourself. SIMS BOS assigns the final color mechanically from estimated_reachable_rank.',
    'Your job is evidence collection and a realistic estimated reachable rank for a well-written personal-blog article on the configured site.',
    'If the query lacks a coherent independent search intent, has no convincing demand signal, or is otherwise unsuitable for a standalone article, set hard_block=true and explain block_reason.',
    'For source=GENERATED_4WORD, confirm a real demand signal from the SERP, suggestions, Q&A, forums, or equivalent evidence.',
    'checked_count must be at least 20 and serp_top20 must contain at least 20 ranked entries for every candidate.',
    'The sources array must contain the important source URLs used for the judgment when available.',
    '',
    'SIMS grade mapping (reference only; do not output the grade):',
    '- 1-10 => GREEN',
    '- 11-20 => YELLOW',
    '- 21-30 => PALE_PINK',
    '- 31+ => RED',
    '- hard_block=true => BLOCK regardless of rank',
    '',
...
function sbosSerpResultJsonSchema_() {
  const serpItem = {
    type:'object',
    properties:{rank:{type:'number'},title:{type:'string'},url:{type:'string'},site_type:{type:'string'},weakness_or_strength:{type:'string'}},
    required:['rank','title','url'],additionalProperties:true
  };
  const result = {
    type:'object',
    properties:{
      ...
      serp_top20:{type:'array', items:serpItem},
      estimated_reachable_rank:{type:'number'},
      ...
      new_site_fit_score:{type:'number'},
      new_site_dimensions:{type:'object'},
      new_site_assessment:{type:'string'}
    },
    required:['rank','main_keyword','checked_count','serp_top20','estimated_reachable_rank','confidence_percent','hard_block','block_reason','blue_ocean_score','search_intent','evidence_summary','exact_or_near_exact_competitors','big_site_pressure','intent_gap','personal_blog_chance','suggested_four_word_queries','sources'],
...
function sbosApplyCannibalReviewPayload_(payload, source) {
  source=source||{};
  if(!payload||payload.format!=='SIMS_BOS_CANNIBAL_REVIEW_RESULT_V1')throw new Error('SIMS_BOS_CANNIBAL_REVIEW_RESULT_V1 形式の結果JSONではありません。');
  if(!Array.isArray(payload.results)||!payload.results.length)throw new Error('results[] が空です。');
  const expectedRequestId=sbosGetState_('cannibal_request_id')||'';
  if(expectedRequestId&&String(payload.request_id||'')!==expectedRequestId)throw new Error('request_idが一致しません。\n期待値: '+expectedRequestId+'\n受信値: '+String(payload.request_id||'未設定'));
  const sh=SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATES);
  if(!sh||sh.getLastRow()<2)throw new Error('Candidatesシートに候補がありません。');
  const data=sh.getRange(2,1,sh.getLastRow()-1,30).getValues(),resultMap=new Map();
  payload.results.forEach(r=>{const key=sbosKeywordMatchKey_(r.main_keyword||'');if(!key)return;if(resultMap.has(key))throw new Error('結果に同一main_keywordが重複しています: '+r.main_keyword);resultMap.set(key,r);});
  let applied=0;const detail={},counts={GREEN:0,YELLOW:0,PALE_PINK:0,RED:0,BLOCK:0};
  data.forEach(row=>{
    const key=sbosKeywordMatchKey_(row[2]),r=resultMap.get(key);if(!r)return;
    const d=String(r.decision||'').toUpperCase(),can=String(r.cannibalization||'').toUpperCase();
    ...
    const serpGrade=String(row[12]||'').toUpperCase();
    row[6]=can;
    const add=String(r.evidence_summary||'');if(add)row[8]=String(row[8]||'')+(row[8]?'\n\n[Cannibal Review] ':'[Cannibal Review] ')+add;
    if(d==='BLOCK'||can==='HIGH'){row[1]='BLOCK';row[29]=add||'カニバリ強制停止';}
    else if(['GREEN','YELLOW','PALE_PINK'].includes(serpGrade)){row[1]=serpGrade;}
    else row[1]=serpGrade||'BLOCK';
    const st=sbosStatusCode_(row[1]);if(Object.prototype.hasOwnProperty.call(counts,st))counts[st]++;
    detail[key]=r;applied++;
  });
  if(!applied)throw new Error('Candidatesと一致するカニバリ精査結果がありませんでした。');
  sh.getRange(2,1,data.length,30).setValues(data);sbosApplyCandidateFormatting_();
  if(source.sourceId)sbosSetState_('cannibal_result_file_id',source.sourceId);sbosSetState_('cannibal_result_source',source.sourceType||'UNKNOWN');sbosSetState_('cannibal_result_source_name',source.sourceName||'');sbosSetState_('cannibal_review_result_json',JSON.stringify(detail));
  const eligible=counts.GREEN+counts.YELLOW+counts.PALE_PINK;sbosSetState_('status',eligible?'CANNIBAL_REVIEW_IMPORTED':SBOS_STATUS.COMPLETE);
  sbosSetHomeStatus_('カニバリ精査結果登録済み（GREEN '+counts.GREEN+' / YELLOW '+counts.YELLOW+' / PALE PINK '+counts.PALE_PINK+' / BLOCK '+counts.BLOCK+'）');
  sbosSyncCandidatePool_();sbosSaveCurrentBlogSession_();
  return {applied:applied,green:counts.GREEN,yellow:counts.YELLOW,palePink:counts.PALE_PINK,red:counts.RED,block:counts.BLOCK,tryCount:0};
}
...
function sbosCreateCannibalReviewPackageFromEvidence(fileId) {
  ...
  try {
    ...
    const sh = SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATES);
    if (!sh || sh.getLastRow() < 2) throw new Error('Candidatesシートに候補がありません。');
    const vals = sh.getRange(2,1,sh.getLastRow()-1,30).getDisplayValues();
    const candidates = vals.filter(r => ['GREEN','YELLOW','PALE_PINK'].includes(String(r[12]||'').toUpperCase()) && sbosStatusCode_(r[1]) === 'CANNIBAL_PENDING').map(r => ({
      rank:Number(r[17])||0,main_keyword:r[2],words:Number(r[3])||0,serp_grade:String(r[12]||''),estimated_reachable_rank:Number(r[27])||0,blue_ocean_score:Number(r[5])||0,search_intent:r[7],evidence_summary:r[8],intent_key:r[11],review_mode:'FINAL_CANNIBAL_GATE'
    }));
    if (!candidates.length) throw new Error('カニバリ精査できるGREEN / YELLOW / PALE PINK候補がありません。');
    ...
    const md = [
      ...
      '## 目的','',
      'SERPでGREEN / YELLOW / PALE PINKとなった候補について、対象サイトの既存記事と検索意図を食い合わず新規記事として独立できるかを最終確認してください。色はSERP到達見込みを表すため、カニバリ確認では色を変更せず、強制停止が必要かを判定します。','',
      '## 判定','',
      ...
      '- 単語が重なるだけではBLOCKにしない',
      '- GREEN / YELLOW / PALE PINK候補とも、aCreatorへ渡せる「新記事が担当する範囲」「既存記事へ任せる範囲」「内部リンク候補」を返す',
      '- cannibalization=HIGH または decision=BLOCK の場合は、新記事化を止める根拠を明示する','',
      '## 返却JSON','',
...
// ============================================================================
const SBOS_CREATOR_BATCH_MAX = 1;
...
function sbosGetCheckedCandidateRows_() {
  const sh = SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATES);
  if (!sh || sh.getLastRow() < 2) return [];
  const vals = sh.getRange(2,1,sh.getLastRow()-1,30).getValues();
...
function sbosQueueSelectedCreatorCases() {
  ...
  if (!selected.length) throw new Error('CandidatesシートでaCreatorへ送るGREEN / YELLOW / PALE PINK候補を1件チェックしてください。');
  if (selected.length > SBOS_CREATOR_BATCH_MAX) throw new Error('1回にaCreator依頼待ちへ登録できるのは1件だけです。現在 ' + selected.length + '件選択されています。');

  selected.forEach(x=>{
    if (!sbosIsCreatorEligibleStatus_(x.values[1])) throw new Error('GREEN / YELLOW / PALE PINK以外の候補が含まれています: ' + x.values[2]);
  ...
  sbosShowWorkflowResult_(
    'aCreator依頼待ちへ登録しました',
    '<b>登録:</b> 1件<br><br>案件は保存されています。次回はCandidatesで「依頼待ち」の案件を1件チェックし、「7. GREEN / YELLOW / PALE PINK候補をaCreatorで処理」から続けられます。',
    '4. 候補・進捗を確認',
    'sbosOpenCandidates'
  );
}
...
function sbosShowSelectedCreatorReferrals() {
  ...
  if (selected.length > SBOS_CREATOR_BATCH_MAX) throw new Error('aCreator依頼文は1回1件だけ表示できます。');
...
function sbosMarkSelectedCreatorCasesDoneFromMenu() {
  ...
  if (selected.length > SBOS_CREATOR_BATCH_MAX) throw new Error('aCreator処置済みの記録は1回1件だけです。');
  selected.forEach(x => {
    if (!sbosIsCreatorEligibleStatus_(x.values[1])) throw new Error('GREEN / YELLOW / PALE PINK以外の候補が含まれています: ' + x.values[2]);
...
function sbosBuildCreatorReferral_(v) {
  ...
  return [
    ...
    '## 最終判定', v[1] + ' / 推定到達順位: ' + (v[27] || '未記録') + '位 / Cannibalization: ' + v[6],'',
    '## 検索意図', v[7],'',
...
function sbosFindCandidateRowByKeyword_(keyword) {
  ...
  const vals=sh.getRange(2,1,sh.getLastRow()-1,30).getValues(), key=sbosKeywordMatchKey_(keyword);
...
function sbosShowWorkflowResult_(title, bodyHtml, primaryLabel, primaryFunction) {
  ...
  const html = HtmlService.createHtmlOutput(
    ...
    (fn ? '<button id="nextBtn" class="primary" onclick="nextStep(this)">' + label + '</button>' : '') +
    '<button class="secondary" onclick="google.script.host.close()">閉じる</button></div></div>' +
    '<script>function nextStep(b){document.getElementById("spin").style.display="flex";if(b){b.disabled=true;b.dataset.o=b.textContent;b.textContent="処理中…";}google.script.run.withSuccessHandler(function(){google.script.host.close();}).withFailureHandler(function(e){document.getElementById("spin").style.display="none";if(b){b.disabled=false;b.textContent=b.dataset.o||"次へ";}alert(e&&e.message?e.message:e);}).' + fn + '();}</script>' +
    '</body></html>'
...
function sbosRegisterSbmArticleResult() {
  ...
  if (!selected.length) {
    SpreadsheetApp.getUi().alert('Candidatesシートで、SIMS Manager登録結果を記録するGREEN / YELLOW / PALE PINK候補にチェックを入れてください。');
    return;
  }
  ...
  if (!sbosIsCreatorEligibleStatus_(v[1])) {
    SpreadsheetApp.getUi().alert('SIMS Manager登録結果はGREEN / YELLOW / PALE PINK候補に記録してください。現在の判定: ' + v[1]);
    return;
  }
  if (String(v[10] || '') !== '作成済み') {
...
function sbosEnsureSheets_() {
  ...
  const defs = [
    ...
    [SBOS_SHEETS.CANDIDATE_POOL, false],
    [SBOS_SHEETS.CANDIDATE_HISTORY, true]
  ];
  ...
  sbosInitKeywords_();
  sbosInitCandidates_();
  sbosInitCandidatePool_();
  sbosInitCandidateHistory_();
  sbosMigrateSessionCandidatesToPool_();
...
function sbosApplyUserVisibleSheets_() {
  const ss = SpreadsheetApp.getActive();
  const visible = new Set([SBOS_SHEETS.HOME, SBOS_SHEETS.KEYWORDS, SBOS_SHEETS.CANDIDATES, SBOS_SHEETS.CANDIDATE_POOL]);
  ...
  [SBOS_SHEETS.HOME, SBOS_SHEETS.KEYWORDS, SBOS_SHEETS.CANDIDATES, SBOS_SHEETS.CANDIDATE_POOL].forEach((name, i) => {
    const sh = ss.getSheetByName(name);
    if (!sh) return;
    try {
...
function sbosEnsureLightweightHome_(force) {
  ...
  const needsBuild = force || signature !== SBOS_PRODUCT_NAME || String(sh.getRange('D1').getDisplayValue() || '') !== 'Version ' + SBOS_VERSION;
  ...
  if (needsBuild) {
    ...
    const rows = [
      ...
      ['入力ファイル','','','現在の状態','','','','','',''],
      ['キーワード探索','','','','','','','','',''],
      ['総キーワード',0,'2語',0,'3語',0,'4語',0,'生成4語',0],
      ['現在の候補状況','','','','','','','','',''],
      ['SERP精査待ち',0,'カニバリ精査待ち',0,'GREEN',0,'aCreator依頼可能',0,'',''],
      ['最終判定','','','','','','','','',''],
      ['GREEN',0,'YELLOW',0,'PALE PINK',0,'RED',0,'',''],
      ['aCreator対象案件の進捗','','','','','','','','',''],
      ['未依頼',0,'aCreator依頼キュー',0,'aCreator処置済み',0,'SIMS Manager登録済み',0,'',''],
      ['標準フロー','','','','','','','','',''],
      ['1 対象サイト設定','2 キーワード読込','3 候補探索','4 候補確認','5 SERP精査','6 カニバリ精査','7 aCreator処理','保存済み候補・再評価','',''],
      ['','','','','','','','','',''],
      ['','','','','','','','','',''],
      ['色の見方','','','','','','','','',''],
      ['GREEN：推定1〜10位','','YELLOW：推定11〜20位','','PALE PINK：推定21〜30位','','RED：31位以下','','',''],
      ...
      ['・GREEN / YELLOW / PALE PINKの未処理候補はCandidate Poolへ保存し、後から最新SERPで再評価できます。','','','','','','','','','']
    ];
    if (sbosIsNewSiteMode_()) {
      rows[1] = ['モード','','','対象サイト','','','','','',''];
      rows[5] = ['現在の候補状況','','','','','','','','',''];
      ...
      rows[20] = ['・最終色は推定到達順位でBOSが判定します。AIは上位30件SERPの証拠収集と順位推定を担当します。','','','','','','','','',''];
...
function sbosInitCandidates_() {
  ...
  sh.getRange(1,1,1,30).setValues([[
...
// ============================================================================
// Candidate Pool / Re-evaluation (v0.14.0)
// ============================================================================
function sbosInitCandidatePool_() {
  const sh=SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATE_POOL); if(!sh)return; if(sh.getMaxColumns()<12)sh.insertColumnsAfter(sh.getMaxColumns(),12-sh.getMaxColumns());
  const headers=['選択','対象サイト','メインキーワード','語数','現在判定','推定到達順位','最終SERP評価日','aCreator状態','Source','判定根拠','Site Key','最終更新'];
  if(sh.getLastRow()===0||String(sh.getRange(1,1).getValue())!=='選択'){sh.clear();sh.getRange(1,1,1,headers.length).setValues([headers]);}
  sh.getRange(1,1,1,headers.length).setFontWeight('bold').setBackground('#e8f0fe').setFontColor('#174ea6');sh.setFrozenRows(1);
  if(sh.getLastRow()>=2)sh.getRange(2,1,sh.getLastRow()-1,1).insertCheckboxes();
  sh.setColumnWidth(1,65);sh.setColumnWidth(2,160);sh.setColumnWidth(3,360);sh.setColumnWidth(5,110);sh.setColumnWidth(6,110);sh.setColumnWidth(7,150);sh.setColumnWidth(8,120);sh.setColumnWidth(10,420);if(sh.getLastRow()>=2){sh.getRange(2,10,sh.getLastRow()-1,1).setWrap(true).setVerticalAlignment('top');try{sh.autoResizeRows(2,sh.getLastRow()-1);}catch(e){}}try{sh.hideColumns(11);}catch(e){}
}
function sbosInitCandidateHistory_(){const sh=SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATE_HISTORY);if(!sh)return;const h=['Site Key','Site','Keyword','Evaluated At','Stage','Status','Estimated Reachable Rank','Evidence'];if(sh.getLastRow()===0){sh.getRange(1,1,1,h.length).setValues([h]).setFontWeight('bold');}if(!sh.isSheetHidden())sh.hideSheet();}
function sbosMigrateSessionCandidatesToPool_(){
  const ss=SpreadsheetApp.getActive(),pool=ss.getSheetByName(SBOS_SHEETS.CANDIDATE_POOL),store=ss.getSheetByName(SBOS_SHEETS.SESSION_CANDIDATES);if(!pool||!store||pool.getLastRow()>1||store.getLastRow()<2)return;
  const meta=ss.getSheetByName(SBOS_SHEETS.BLOG_SESSIONS),siteNames={};if(meta&&meta.getLastRow()>=2)meta.getRange(2,1,meta.getLastRow()-1,3).getDisplayValues().forEach(r=>siteNames[String(r[0]||'')]=String(r[1]||r[2]||r[0]||''));
  const rows=store.getRange(2,1,store.getLastRow()-1,3).getValues(),out=[],seen={};rows.forEach(r=>{if(Number(r[1])<=1)return;let v;try{v=JSON.parse(String(r[2]||'[]'));}catch(e){return;}if(!Array.isArray(v)||!v[2])return;const st=sbosStatusCode_(v[1]);if(st==='BLOCK')return;const siteKey=String(r[0]||''),key=siteKey+'|'+sbosKeywordMatchKey_(v[2]);if(seen[key])return;seen[key]=true;const poolStatus=st==='CANNIBAL_PENDING'?String(v[12]||st):st;out.push([false,siteNames[siteKey]||siteKey,String(v[2]),v[3]||'',poolStatus,v[27]||'',v[28]||'',v[10]||'',v[9]||'',v[8]||'',siteKey,sbosNow_()]);});if(out.length){pool.getRange(2,1,out.length,12).setValues(out);pool.getRange(2,1,out.length,1).insertCheckboxes();}
}
function sbosPoolSiteKey_(){return sbosCurrentSiteKey_()||(sbosIsNewSiteMode_()?'__NEW_SITE__':'__UNASSIGNED__');}
function sbosAppendCandidateHistory_(keyword,status,estimatedRank,when,stage,evidence){const sh=SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATE_HISTORY);if(!sh)return;sh.appendRow([sbosPoolSiteKey_(),sbosGetSetting_('site_name')||'',String(keyword||''),String(when||sbosNow_()),String(stage||''),String(status||''),estimatedRank||'',String(evidence||'')]);}
function sbosCandidatePoolIsGraduated_(creatorState,articleId){const st=String(creatorState||'').trim();return st==='作成済み'||st==='SIMS Manager登録済み'||st==='SBM登録済み'||!!String(articleId||'').trim();}
function sbosCandidatePoolShouldExclude_(status,creatorState,articleId){const st=sbosStatusCode_(status);return ['RED','BLOCK','CLUSTERED'].includes(st)||sbosCandidatePoolIsGraduated_(creatorState,articleId);}
function sbosSyncCandidatePool_(){
  const ss=SpreadsheetApp.getActive(),src=ss.getSheetByName(SBOS_SHEETS.CANDIDATES),pool=ss.getSheetByName(SBOS_SHEETS.CANDIDATE_POOL);if(!src||!pool||src.getLastRow()<2)return {upserted:0,removed:0};sbosInitCandidatePool_();
  const siteKey=sbosPoolSiteKey_(),site=sbosGetSetting_('site_name')|| (sbosIsNewSiteMode_()?'新規サイト探索':'未設定');
  const current=src.getRange(2,1,src.getLastRow()-1,30).getDisplayValues();
  let removed=0;
  if(pool.getLastRow()>=2){for(let rr=pool.getLastRow();rr>=2;rr--){const pv=pool.getRange(rr,1,1,12).getValues()[0];if(String(pv[10]||'')===siteKey&&sbosCandidatePoolShouldExclude_(pv[4],pv[7],'')){pool.deleteRow(rr);removed++;}}}
  const existing=pool.getLastRow()>=2?pool.getRange(2,1,pool.getLastRow()-1,12).getValues():[];const map={};existing.forEach((r,i)=>map[String(r[10]||'')+'|'+sbosKeywordMatchKey_(r[2])]=i+2);
  let upserted=0;
  current.forEach(r=>{const kw=String(r[2]||'').trim();if(!kw)return;const st=sbosStatusCode_(r[1]),poolStatus=st==='CANNIBAL_PENDING'?String(r[12]||st):st,key=siteKey+'|'+sbosKeywordMatchKey_(kw),hit=map[key];if(sbosCandidatePoolShouldExclude_(poolStatus,r[10],r[13])){if(hit){pool.deleteRow(hit);removed++;Object.keys(map).forEach(k=>{if(map[k]>hit)map[k]--;});delete map[key];}return;}const row=[false,site,kw,r[3],poolStatus,r[27]||'',r[28]||'',r[10]||'',r[9]||'',r[8]||'',siteKey,sbosNow_()];if(hit)pool.getRange(hit,1,1,12).setValues([row]);else{pool.appendRow(row);map[key]=pool.getLastRow();}upserted++;});
  if(pool.getLastRow()>=2){pool.getRange(2,1,pool.getLastRow()-1,1).insertCheckboxes();pool.getRange(2,10,pool.getLastRow()-1,1).setWrap(true).setVerticalAlignment('top');try{pool.autoResizeRows(2,pool.getLastRow()-1);}catch(e){}}
  try{sbosApplyCandidatePoolSiteFilter_();}catch(e){}
  return {upserted:upserted,removed:removed};
}
function sbosCandidatePoolContextKey_(){return sbosPoolSiteKey_();}
function sbosCandidatePoolContextLabel_(){
  if(sbosIsNewSiteMode_())return '対象サイト指定なし（新規サイト探索）';
  const site=String(sbosGetSetting_('site_name')||'').trim();
  return site||'対象サイト指定なし';
}
function sbosCandidatePoolCountForCurrentContext_(){
  const sh=SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATE_POOL);if(!sh||sh.getLastRow()<2)return 0;
  const key=sbosCandidatePoolContextKey_();
  return sh.getRange(2,11,sh.getLastRow()-1,1).getDisplayValues().filter(r=>String(r[0]||'')===key).length;
}
function sbosApplyCandidatePoolSiteFilter_(){
  const sh=SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATE_POOL);if(!sh)return {visible:0,hidden:0,key:sbosCandidatePoolContextKey_()};
  const last=sh.getLastRow();if(last<2)return {visible:0,hidden:0,key:sbosCandidatePoolContextKey_()};
  try{sh.showRows(2,last-1);}catch(e){}
  const key=sbosCandidatePoolContextKey_(),vals=sh.getRange(2,1,last-1,12).getValues();
  let visible=0,hidden=0,start=null,len=0;
  const flush=()=>{if(start!==null&&len>0){try{sh.hideRows(start,len);}catch(e){}hidden+=len;start=null;len=0;}};
  vals.forEach((r,i)=>{
    const row=i+2,same=String(r[10]||'')===key;
    if(same){visible++;flush();}
    else{
      if(r[0]===true)sh.getRange(row,1).setValue(false);
      if(start===null){start=row;len=1;}else if(start+len===row){len++;}else{flush();start=row;len=1;}
    }
  });
  flush();
  return {visible:visible,hidden:hidden,key:key};
}
function sbosOpenCandidatePool(){
  sbosEnsureSheets_();sbosSyncCandidatePool_();
  const sh=SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATE_POOL),result=sbosApplyCandidatePoolSiteFilter_();
  SpreadsheetApp.getActive().setActiveSheet(sh);sh.setActiveSelection('A1');
  sbosSetHomeStatus_('Candidate Pool: '+sbosCandidatePoolContextLabel_()+' / '+result.visible+'件');
}
function sbosGetSelectedPoolCandidate_(){
  const sh=SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATE_POOL);if(!sh||sh.getLastRow()<2)return null;
  const key=sbosCandidatePoolContextKey_(),v=sh.getRange(2,1,sh.getLastRow()-1,12).getValues();
  const a=v.map((r,i)=>({row:i+2,values:r})).filter(x=>x.values[0]===true&&String(x.values[10]||'')===key);
  if(a.length!==1)throw new Error('現在の対象サイトのCandidate Poolから、再評価する候補を1件だけチェックしてください。');
  return a[0];
}
function sbosReevaluationRequestText_(keyword,requestId){return ['# SIMS BOS 保存済み候補 SERP再評価','', '対象キーワード: '+keyword,'request_id: '+requestId,'','現在のWeb検索でこのキーワードを検索し、自然検索の上位20件を1位から20位まで実際に確認してください。','AIはGREEN/YELLOW/PALE PINK/REDを決めません。現実的な estimated_reachable_rank を返してください。','独立した検索意図が成立しない、需要Signalが確認できない等の強制停止条件がある場合だけ hard_block=true としてください。','checked_countは20以上、serp_top20は20件以上必須です。','','回答末尾に SIMS_BOS_SERP_REVIEW_RESULT_V1 の完全JSONを ```json``` で出力してください。resultsはこの1件だけです。','必要フィールド: main_keyword, checked_count, serp_top20[{rank,title,url,site_type,weakness_or_strength}], estimated_reachable_rank, confidence_percent, hard_block, block_reason, blue_ocean_score, search_intent, evidence_summary, exact_or_near_exact_competitors, big_site_pressure, intent_gap, personal_blog_chance, suggested_four_word_queries, sources','', '判定基準はSIMS側で固定: 1-10 GREEN / 11-20 YELLOW / 21-30 PALE PINK / 31+ RED / hard_block=true はBLOCK'].join('\n');}
function sbosReevaluateSelectedPoolCandidate(){
  sbosEnsureSheets_();const x=sbosGetSelectedPoolCandidate_(),v=x.values,kw=String(v[2]||''),siteKey=String(v[10]||'');if(siteKey!==sbosPoolSiteKey_())throw new Error('この候補は別サイトの候補です。先に「1. 対象サイトを設定・切り替える」で対象サイトへ切り替えてください。');
  const risk=sbosCannibalRisk_(kw);if(risk.risk==='HIGH'){const sh=SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATE_POOL);sbosAppendCandidateHistory_(kw,'BLOCK','',sbosNow_(),'PRE_REEVALUATION_CANNIBAL','既存記事とのカニバリHIGH: '+risk.matched.map(m=>m.title).join(' | '));sh.deleteRow(x.row);SpreadsheetApp.getUi().alert('再評価を停止しました','現在のサイトデータでカニバリHIGHを検出したためBLOCKとしました。\n\n'+risk.matched.map(m=>'・'+m.title).join('\n'),SpreadsheetApp.getUi().ButtonSet.OK);return;}
  const requestId='SBOS-REEVAL-'+Utilities.formatDate(new Date(),Session.getScriptTimeZone()||'Asia/Tokyo','yyyyMMdd-HHmmss');sbosSetState_('reeval_request_id',requestId);sbosSetState_('reeval_keyword',kw);const req=sbosReevaluationRequestText_(kw,requestId);
  const html=HtmlService.createHtmlOutput('<!doctype html><html><head><base target="_top"><style>body{font-family:Arial;margin:0;color:#202124}.w{padding:18px}.t{font-size:20px;font-weight:700}.m{background:#e8f0fe;padding:10px;border-radius:8px;line-height:1.55;margin:10px 0}textarea{width:100%;box-sizing:border-box;border:1px solid #dadce0;border-radius:7px;padding:10px;font-family:monospace;font-size:12px}#req{height:220px}#ans{height:220px}.a{display:flex;justify-content:flex-end;gap:8px;margin-top:8px}button{border:0;border-radius:6px;padding:9px 14px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:7px}.p{background:#1a73e8;color:#fff}.s{background:#f1f3f4}.sp{display:none;width:14px;height:14px;border:2px solid rgba(255,255,255,.45);border-top-color:#fff;border-radius:50%;animation:r .75s linear infinite}.working .sp{display:inline-block}@keyframes r{to{transform:rotate(360deg)}}.st{background:#f8fafd;padding:8px;border-radius:6px;margin-top:7px;white-space:pre-wrap}</style></head><body><div class="w"><div class="t">保存済み候補を再評価</div><div class="m"><b>キーワード:</b> '+sbosEscapeHtml_(kw)+'<br><b>事前カニバリ:</b> '+sbosEscapeHtml_(risk.risk)+'<br>最新SERP上位20件で再評価します。</div><textarea id="req" readonly>'+sbosEscapeHtml_(req)+'</textarea><div class="a"><button id="cp" class="p" onclick="copyReq(this)"><span class="sp"></span><span class="tx">依頼文をコピー</span></button></div><div id="cst" class="st"></div><textarea id="ans" placeholder="Claude / Geminiの回答全文を貼り付け"></textarea><div id="st" class="st">回答待ち</div><div class="a"><button class="s" onclick="google.script.host.close()">閉じる</button><button id="go" class="p" onclick="reg(this)"><span class="sp"></span><span class="tx">再評価結果を登録</span></button></div><script>function w(b,on,label){const t=b.querySelector(".tx");if(on){b.dataset.o=t.textContent;t.textContent=label||"処理中…";b.classList.add("working");b.disabled=true}else{t.textContent=b.dataset.o||"実行";b.classList.remove("working");b.disabled=false}}async function copyReq(b){w(b,true,"コピー中…");const t=document.getElementById("req");let ok=true;try{await navigator.clipboard.writeText(t.value)}catch(e){try{t.select();ok=document.execCommand("copy")}catch(z){ok=false}}w(b,false);if(ok){b.querySelector(".tx").textContent="✓ コピーしました";document.getElementById("cst").textContent="✓ 依頼文をコピーしました。Claude / Geminiへ貼り付けてください。";setTimeout(()=>b.querySelector(".tx").textContent="依頼文をコピー",2200)}else{document.getElementById("cst").textContent="コピーできませんでした。手動でコピーしてください。"}}function reg(b){const t=document.getElementById("ans").value;if(!t.trim()){document.getElementById("st").textContent="回答全文を貼り付けてください。";return}w(b,true,"登録中…");document.getElementById("st").textContent="再評価結果を検証・登録しています…";google.script.run.withSuccessHandler(r=>{w(b,false);b.querySelector(".tx").textContent="✓ 登録完了";document.getElementById("st").textContent="再評価: "+r.grade+" / 推定到達順位 "+r.estimatedRank+"位\n"+(r.creatorEligible?"次はカニバリ精査後、aCreatorへ進めます。":"aCreator対象外です。")}).withFailureHandler(e=>{w(b,false);document.getElementById("st").textContent="エラー: "+(e&&e.message?e.message:e)}).sbosRegisterPoolReevaluation(document.getElementById("ans").value)}</script></div></body></html>').setWidth(820).setHeight(720);SpreadsheetApp.getUi().showModalDialog(html,'保存済み候補・再評価');
}
function sbosRegisterPoolReevaluation(rawText){
  const payload=sbosExtractContractJsonFromText_(rawText,'SIMS_BOS_SERP_REVIEW_RESULT_V1'),requestId=sbosGetState_('reeval_request_id')||'',kw=sbosGetState_('reeval_keyword')||'';if(requestId&&String(payload.request_id||'')!==requestId)throw new Error('request_idが一致しません。');if(!Array.isArray(payload.results)||payload.results.length!==1)throw new Error('再評価結果は1件だけ返してください。');const r=payload.results[0];if(sbosKeywordMatchKey_(r.main_keyword)!==sbosKeywordMatchKey_(kw))throw new Error('main_keywordが一致しません。');const checked=Number(r.checked_count),top=Array.isArray(r.serp_top20)?r.serp_top20:(Array.isArray(r.serp_top30)?r.serp_top30:[]);if(checked<20||top.length<20)throw new Error('上位20件SERP確認が不足しています。');const est=Number(r.estimated_reachable_rank);if(!isFinite(est)||est<1)throw new Error('estimated_reachable_rankが不正です。');const grade=r.hard_block===true?'BLOCK':sbosSerpGradeFromRank_(est),pool=SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATE_POOL),selected=sbosGetSelectedPoolCandidate_();if(grade==='BLOCK'){pool.deleteRow(selected.row);}else{pool.getRange(selected.row,5).setValue(grade);pool.getRange(selected.row,6).setValue(Math.round(est));pool.getRange(selected.row,7).setValue(payload.reviewed_at||sbosNow_());pool.getRange(selected.row,10).setValue(String(r.evidence_summary||''));pool.getRange(selected.row,12).setValue(sbosNow_());pool.getRange(selected.row,1).setValue(false);}sbosAppendCandidateHistory_(kw,grade,est,payload.reviewed_at||sbosNow_(),'SERP_REEVALUATION',String(r.evidence_summary||''));
  const c=SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATES);let hit=sbosFindCandidateRowByKeyword_(kw);if(!hit){const pv=selected.values;const row=[false,grade,kw,Number(pv[3])||sbosDetectWordCount_(kw),'',Number(r.blue_ocean_score)||'',grade==='BLOCK'?'NOT_RUN':'PENDING',String(r.search_intent||''),String(r.evidence_summary||''),String(pv[8]||'POOL_REEVALUATION'),'未作成',sbosIntentKey_(sbosNormalizeKeyword_(kw)),grade,'','','','',c.getLastRow(),'','','','','','','','','',Math.round(est),payload.reviewed_at||sbosNow_(),grade==='BLOCK'?String(r.block_reason||''):''];c.appendRow(row);hit=sbosFindCandidateRowByKeyword_(kw);}else{const rr=hit.row;c.getRange(rr,6).setValue(Number(r.blue_ocean_score)||'');c.getRange(rr,7).setValue(['GREEN','YELLOW','PALE_PINK'].includes(grade)?'PENDING':'NOT_RUN');c.getRange(rr,8).setValue(String(r.search_intent||hit.values[7]||''));c.getRange(rr,9).setValue(String(r.evidence_summary||''));c.getRange(rr,13).setValue(grade);c.getRange(rr,28).setValue(Math.round(est));c.getRange(rr,29).setValue(payload.reviewed_at||sbosNow_());c.getRange(rr,30).setValue(grade==='BLOCK'?String(r.block_reason||''):'');c.getRange(rr,2).setValue(['GREEN','YELLOW','PALE_PINK'].includes(grade)?sbosStatusLabel_('CANNIBAL_PENDING'):grade);}
  sbosApplyCandidateFormatting_();sbosSyncCandidatePool_();sbosSaveCurrentBlogSession_();return {ok:true,grade:grade,estimatedRank:Math.round(est),creatorEligible:['GREEN','YELLOW','PALE_PINK'].includes(grade)};
}

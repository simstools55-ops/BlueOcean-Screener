...
- SERP精査は **1サイクル最大5候補 × 各自然検索上位20件** です。
- 1サイクルの回答登録後、GREEN＋YELLOWの累計を確認します。
- **GREEN＋YELLOWが10件に到達**、または **5サイクル完了** の早い方でSERP精査を終了します。
- PALE PINK（推定21〜30位）は記事候補として保持しますが、10件目標のカウントには含めません。
- RED（31位以下）とBLOCKは履歴には保存しますが、Candidates / Candidate Poolから除外します。
- Gemini / Claudeともに `serp_top20` へ1位〜20位を省略せず返す契約です。`checked_count=20` の自己申告だけでは登録できません。
- 停止条件に達しない場合は、画面から「次の5候補をSERP精査」へ進めます。


## 1. このバージョンについて

v0.14.6 は、一次選抜30件を一括900結果で精査せず、5候補×上位20件の小さなサイクルへ分割した実運用試験版です。Claude用ZIP / Gemini用Markdownの両方を維持し、GREEN＋YELLOW 10件または5サイクル完了で停止します。aCreator依頼は引き続き1件ずつ処理します。

- AIは各候補について現在の自然検索上位20件を確認し、`estimated_reachable_rank` を返します。
...
- Candidate Poolから1件を選び、最新SERP20件で再評価できます。

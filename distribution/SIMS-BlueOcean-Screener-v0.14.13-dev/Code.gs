 ...
 * v0.14.8: Claude-only SERP review; minimum verified evidence lowered to 5 and own-site SERP hits are excluded from new-article candidates.
 * v0.14.7: SERP evidence review uses reachable bands (TOP10/TOP20/TOP30/OUT) from honest verified results; no pseudo-exact rank required.
...
// v0.14.11: lightweight persistence for SERP answer registration.
// Keywords / Settings are unchanged by a SERP import, so do not serialize them again.
// Candidate Pool synchronization is deliberately NOT performed here; caller does it once.
function sbosSaveSerpSessionFast_() {
  const siteName = sbosGetSetting_('site_name') || '';
  const siteUrl = sbosGetSetting_('site_url') || '';
  const siteKey = sbosSiteKey_(siteName, siteUrl);
  if (!siteKey || (!siteName && !siteUrl)) return {saved:false};
...
// v0.14.11: SERP-import-only formatting. Avoids column hide/show, width/note resets,
...
function sbosApplyCandidateFormattingFast_() {
  ...
  const bgs = vals.map(r => {
    ...
    if (creator === 'SIMS Manager登録済み' || articleId) return ['#eeeeee'];
    if (st === 'GREEN') return ['#d9ead3'];
    if (st === 'CANNIBAL_PENDING' || st === 'YELLOW') return ['#fff2cc'];
    if (st === 'PALE_PINK') return ['#fce8ef'];
    if (st === 'RED') return ['#f4cccc'];
    if (st === 'BLOCK' || st === 'CLUSTERED') return ['#e6e6e6'];
    if (st === 'TRY') return ['#d9eaf7'];
    return ['#f1f3f4'];
  });
...
function sbosCreateSerpReviewPackage(options) {
  ...
  try {
    ...
    const blobs = [
      ...
      Utilities.newBlob(jsonText, 'application/json', 'SERP_REVIEW_REQUEST_V2.json')
    ];
    const outputName = 'SIMS-BOS-' + safeSite + '-Claude-SERP-Review-' + stamp + '.zip';
    const outputBlob = Utilities.zip(blobs, outputName);
...
function sbosBuildSerpWorkflowHtml_(isNewSite, siteName, pending) {
  ...
  return '<!doctype html>' +
    ...
    'function createPackage(){' +
      'var b=q("serpClaudeBtn"),s=q("pkg");' +
      'busy(b,true,"Claude用作成中…");' +
      's.textContent="Claude用ZIPを作成しています…";' +
    ...
    'function registerSerp(){' +
      ...
      'if(!ans.trim()){s.textContent="Claudeの回答全文を貼り付けてください。";return}' +
...
function sbosBuildSerpPackageReadme_(p) {
  return [
    ...
    '各候補について検索で実在確認できた結果だけを収集し、最低5件を目安に競合と検索意図を評価します。',
...
function sbosShowResultPasteDialog_(kind) {
  ...
  const guide = isSerp
    ? 'SERP精査を行ったClaudeの回答を、説明文を含めて全文そのまま貼り付けてください。JSON部分だけを抜き出す必要はありません。'
...
function sbosUrlHost_(url) {
  const m = String(url || '').trim().match(/^https?:\/\/([^\/:?#]+)/i);
  return m ? String(m[1] || '').toLowerCase().replace(/^www\./,'') : '';
}

function sbosEvidenceHasOwnSite_(evidence) {
  if (sbosIsNewSiteMode_()) return false;
  const siteHost = sbosUrlHost_(sbosGetSetting_('site_url') || '');
  if (!siteHost || !Array.isArray(evidence)) return false;
  return evidence.some(e => {
    const host = sbosUrlHost_(e && e.url);
    return host && (host === siteHost || host.endsWith('.' + siteHost) || siteHost.endsWith('.' + host));
  });
}
...
function sbosApplySerpReviewPayload_(payload, source) {
  ...
  let applied=0; const counts={GREEN:0,YELLOW:0,PALE_PINK:0,RED:0,BLOCK:0,EXISTING_ARTICLE:0}; const historyBatch=[];
  data.forEach((row,i)=>{
    const r=resultMap.get(sbosKeywordMatchKey_(row[2])); if(!r)return;
    const reportedChecked=Number(r.checked_count), evidence=Array.isArray(r.serp_evidence)?r.serp_evidence:[];
    const actualChecked=evidence.length;
    if(actualChecked<SBOS_THRESHOLDS.SERP_CHECK_DEPTH) throw new Error('検索証拠が不足しています: '+r.main_keyword+' / serp_evidence='+actualChecked+'（最低'+SBOS_THRESHOLDS.SERP_CHECK_DEPTH+'件）');
    // AIのchecked_countは参考値。BOS自身がserp_evidence.lengthを実確認件数として正規化する。
    r.checked_count=actualChecked;
  ...
  sbosAppendCandidateHistoryBatch_(historyBatch);
  // Reject rows are removed in memory before the only Candidates table write.
  let excluded=0;
  let kept=data.filter(row=>{const st=sbosStatusCode_(row[12]||row[1]);const reject=st==='RED'||st==='BLOCK'||st==='EXISTING_ARTICLE';if(reject)excluded++;return !reject;});
  kept.sort((a,b)=>{const p=sbosCandidateSortPriority_(a)-sbosCandidateSortPriority_(b);if(p)return p;return Number(b[5]||b[4]||0)-Number(a[5]||a[4]||0);});
  kept.forEach((r,i)=>r[17]=i+1);
  if(sh.getLastRow()>1)sh.getRange(2,1,sh.getLastRow()-1,30).clearContent();
...
function sbosPruneRejectedCandidates_(removeClustered) {
  ...
  data.forEach(row => {
    ...
    const reject = st === 'RED' || st === 'BLOCK' || st === 'EXISTING_ARTICLE' || (removeClustered === true && st === 'CLUSTERED');
    if (reject) removed++;
    else kept.push(row);
  });
  if (sh.getLastRow() > 1) sh.getRange(2,1,sh.getLastRow()-1,30).clearContent();
...
function sbosAppendCandidateHistoryBatch_(items){const sh=SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATE_HISTORY);if(!sh||!Array.isArray(items)||!items.length)return;const siteKey=sbosPoolSiteKey_(),site=sbosGetSetting_('site_name')||'',rows=items.map(x=>[siteKey,site,String(x.keyword||''),String(x.when||sbosNow_()),String(x.stage||''),String(x.status||''),x.reachableBand||'',String(x.evidence||'')]);sh.getRange(sh.getLastRow()+1,1,rows.length,8).setValues(rows);}
...
// v0.14.11: one-pass Candidate Pool rebuild for the current site during SERP import.
// Avoids deleteRow/appendRow loops, autoResizeRows and background row hide/show work.
function sbosSyncCandidatePoolFast_(){
  const ss=SpreadsheetApp.getActive(),src=ss.getSheetByName(SBOS_SHEETS.CANDIDATES),pool=ss.getSheetByName(SBOS_SHEETS.CANDIDATE_POOL);
  if(!src||!pool)return {upserted:0,removed:0};
  const siteKey=sbosPoolSiteKey_(),site=sbosGetSetting_('site_name')||(sbosIsNewSiteMode_()?'新規サイト探索':'未設定'),now=sbosNow_();
  const old=pool.getLastRow()>=2?pool.getRange(2,1,pool.getLastRow()-1,12).getValues():[];
  const other=old.filter(r=>String(r[10]||'')!==siteKey);
  const previousCurrent=old.length-other.length;
  const outCurrent=[];
  if(src.getLastRow()>=2){
    src.getRange(2,1,src.getLastRow()-1,30).getDisplayValues().forEach(r=>{
      const kw=String(r[2]||'').trim(); if(!kw)return;
      const st=sbosStatusCode_(r[1]),poolStatus=st==='CANNIBAL_PENDING'?String(r[12]||st):st;
      if(sbosCandidatePoolShouldExclude_(poolStatus,r[10],r[13]))return;
      outCurrent.push([false,site,kw,r[3],poolStatus,r[27]||'',r[28]||'',r[10]||'',r[9]||'',r[8]||'',siteKey,now]);
    });
  }
  const all=other.concat(outCurrent);
  if(pool.getLastRow()>1)pool.getRange(2,1,pool.getLastRow()-1,12).clearContent();
  if(all.length){pool.getRange(2,1,all.length,12).setValues(all);pool.getRange(2,1,all.length,1).insertCheckboxes();pool.getRange(2,10,all.length,1).setWrap(true).setVerticalAlignment('top');}
  return {upserted:outCurrent.length,removed:Math.max(0,previousCurrent-outCurrent.length)};
}
...
function sbosRegisterPoolReevaluation(rawText){
  ...
  const reportedChecked=Number(r.checked_count),evidence=Array.isArray(r.serp_evidence)?r.serp_evidence:[];
  const actualChecked=evidence.length;
  if(actualChecked<SBOS_THRESHOLDS.SERP_CHECK_DEPTH)throw new Error('検索証拠が不足しています。serp_evidenceは最低'+SBOS_THRESHOLDS.SERP_CHECK_DEPTH+'件必要です。');
  // 再評価でもAI申告値ではなく、実際のserp_evidence件数を正本とする。
  r.checked_count=actualChecked;

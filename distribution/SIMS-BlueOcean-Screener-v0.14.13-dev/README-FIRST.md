# SIMS Blue Ocean Screener v0.14.13

## v0.14.13 aCreator → SIMS Manager 順次登録UI
- ② aCreator回答を登録するまでは③ SIMS Manager登録結果を表示しない。
- aCreator回答登録成功後に、SIMS Managerへ新記事を登録してArticle ID（Axxxxxx）を取得するよう案内する。
- ③ではArticle IDを必須、公開URLを任意としてBOSへ登録結果を記録する。
- Article IDは `A` + 6桁の形式を検証する。

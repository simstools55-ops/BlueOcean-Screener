...
- 各候補は、AIがWeb検索で**実在確認できた検索結果を最低8件目安**に収集します。
- `checked_count` は実際の確認件数、`serp_evidence` は実在確認済みページだけを返します。両者の件数一致をBOSが検証します。
...
- Claude用ZIP / Gemini用Markdownの両方で同じV2契約を使用します。


## 1. このバージョンについて

v0.14.7 は、AIに擬似的なGoogle実順位を作らせず、実在確認できた検索証拠と到達見込み帯で評価する実運用試験版です。Claude用ZIP / Gemini用Markdownの両方を維持し、GREEN＋YELLOW 10件または5サイクル完了で停止します。aCreator依頼は引き続き1件ずつ処理します。

- AIは各候補について実在確認できた検索結果を最低8件目安で確認し、`reachable_band` を返します。
...
2. キーワードを読み込み
3. 一次選抜・3語／4語候補を生成
4. SERP精査依頼を作成（Claude用ZIP / Gemini用Markdown）
5. Claude / Gemini回答を登録
6. 必要な候補をカニバリ精査
...
SERP精査では、画面から Claude用ZIP または Gemini用Markdown を作成できます。カニバリ精査は従来どおりClaude Packageを使用します。

- 「Claude用ZIPを作成」または「Gemini用ファイルを作成」を押すと、直ちに処理中表示とスピナーが出ます。
- 完了するまで同じボタンを繰り返し押さないでください。
- Claudeを使う場合は生成ZIPをClaudeへ、Geminiを使う場合は生成MarkdownをGeminiへアップロードします。
- Claude / Geminiから返された回答全文を、同じ回答欄へそのまま貼り付けて登録します。

/**
 * Blue Ocean Screener v0.14.7
 * Single-Code Apps Script distribution.
 * v0.14.7: SERP evidence review uses reachable bands (TOP10/TOP20/TOP30/OUT) from honest 8+ verified results; no pseudo-exact rank required.
...
function sbosCreateSerpReviewPackage(options) {
  ...
  try {
    ...
    const payload = {
      format: 'SIMS_BOS_SERP_REVIEW_REQUEST_V2',
      ...
      cycle: {number: progressBefore.completedCycles + 1, batch_size: SBOS_THRESHOLDS.SERP_BATCH_SIZE, minimum_verified_results: SBOS_THRESHOLDS.SERP_CHECK_DEPTH, max_cycles: SBOS_THRESHOLDS.SERP_MAX_CYCLES, target_green_yellow: SBOS_THRESHOLDS.SERP_TARGET_YELLOW_PLUS},
      evaluation_policy: isNewSite ? {
        ...
        minimum_verified_results_per_candidate: SBOS_THRESHOLDS.SERP_CHECK_DEPTH,
        prohibit_fabricated_or_inferred_results: true,
        final_grade_owner: 'SIMS_BOS',
        grade_mapping: 'TOP10 GREEN / TOP20 YELLOW / TOP30 PALE_PINK / OUT RED / hard_block=true => BLOCK',
...
function sbosBuildSerpWorkflowHtml_(isNewSite, siteName, pending) {
  ...
  return '<!doctype html>' +
    ...
    '1サイクルは最大5候補です。各候補は実在確認できた検索証拠を最低8件目安で評価します。停止条件はGREEN＋YELLOW 10件、または5サイクル完了です。<br>' +
...
function sbosBuildSerpPackageReadme_(p) {
  return [
    ...
    '各候補について検索で実在確認できた結果だけを収集し、最低8件を目安に競合と検索意図を評価します。',
    'Googleの厳密な1位〜20位を取得できない場合、順位を推測・補完・捏造しないでください。',
...
function sbosBuildSerpReviewRequestMarkdown_(p) {
  ...
  const lines = [
    ...
    '- AIは検索で実在確認できた結果だけを証拠として返す',
    '- Googleの厳密な順位を取得できない場合は rank を推測・補完しない',
    '- 架空URL、サイト内検索URLを個別SERP結果の代用品として並べることは禁止',
    '- AIは十分に作り込んだ個人ブログ記事の到達見込みを reachable_band で返す',
    '- reachable_band: TOP10 / TOP20 / TOP30 / OUT',
    '- BOSが reachable_band から最終色を機械判定する',
    '- TOP10: GREEN / TOP20: YELLOW / TOP30: PALE PINK / OUT: RED',
    '- 独立検索意図が成立しない、需要Signalが確認できない等の強制停止条件だけ hard_block=true とする','',
    '## 検索証拠の必須条件','',
    '- checked_count は実際に確認できた件数を正直に返す',
    '- serp_evidence は最低' + SBOS_THRESHOLDS.SERP_CHECK_DEPTH + '件を目安とし、実在確認できた結果だけを入れる',
    '- serp_evidence[] は title, url を必須とし、rank は実順位を確認できた場合だけ付ける',
    '- 取得できなかった件数を埋めるための推測・創作・重複は禁止',
    '- 企業・公式・大手メディアによる占有度',
    '- 個人ブログ、Q&A、フォーラム等の混在',
  ...
  if (isNew) {
    lines.push('## 新規サイト向け追加評価','',
      ...
      '- new_site_dimensions {entry_ease,demand,serp_gap,expansion,cluster_potential,continuity,risk}',
      '- new_site_assessment',
      'これらは補助評価です。最終色自体は reachable_band をBOSが4段階へ変換します。','');
  } else {
    lines.push('## 対象サイトとの関係','',
      '対象サイトで十分に作り込んだ新記事を公開した場合の現実的な到達レンジを評価してください。',
      'SERP精査段階では既存記事との最終カニバリ判定を行わず、GREEN / YELLOW / PALE PINKは後段のカニバリ精査へ送ります。','');
  }
  lines.push('## 返却JSON','',
    '回答末尾に `SIMS_BOS_SERP_REVIEW_RESULT_V2` の完全JSONを ```json コードブロックでインライン出力してください。',
...
function sbosBuildGeminiSerpReviewMarkdown_(p) {
  return [
    ...
    'GeminiのWeb検索機能を使い、各候補について実在確認できた検索結果だけを収集してください。',
    '1回の依頼は最大5候補です。最低' + SBOS_THRESHOLDS.SERP_CHECK_DEPTH + '件を目安に証拠を集め、取得できた実数を checked_count に記録してください。',
    'Googleの厳密な順位を確認できない場合はrankを推測しないでください。架空URL・未確認結果・サイト内検索URLによる穴埋めは禁止です。',
    '推定の精密順位ではなく reachable_band（TOP10 / TOP20 / TOP30 / OUT）を返してください。',
    'GREEN / YELLOW / PALE PINK / RED の色判定は行わず、最終色はBOSが機械判定します。',
    '回答末尾に SIMS_BOS_SERP_REVIEW_RESULT_V2 の完全JSONを必ずインライン出力してください。','',
    '---','',
...
function sbosImportSerpReviewText(rawText) {
  const payload = sbosExtractContractJsonFromText_(rawText, 'SIMS_BOS_SERP_REVIEW_RESULT_V2');
...
function sbosApplySerpReviewPayload_(payload, source) {
  source = source || {};
  if (!payload || payload.format !== 'SIMS_BOS_SERP_REVIEW_RESULT_V2') throw new Error('SIMS_BOS_SERP_REVIEW_RESULT_V2 形式のSERP結果ではありません。');
  ...
  payload.results.forEach(r => { const key=sbosKeywordMatchKey_(r.main_keyword||''); if(!key)return; if(resultMap.has(key))throw new Error('SERP結果に同一main_keywordが重複しています: '+String(r.main_keyword||'')); resultMap.set(key,r); });
  ...
  let applied=0; const counts={GREEN:0,YELLOW:0,PALE_PINK:0,RED:0,BLOCK:0};
  data.forEach((row,i)=>{
    const r=resultMap.get(sbosKeywordMatchKey_(row[2])); if(!r)return;
    const checked=Number(r.checked_count), evidence=Array.isArray(r.serp_evidence)?r.serp_evidence:[];
    if(!isFinite(checked)||checked<SBOS_THRESHOLDS.SERP_CHECK_DEPTH||evidence.length<SBOS_THRESHOLDS.SERP_CHECK_DEPTH) throw new Error('検索証拠が不足しています: '+r.main_keyword+' / checked_count='+r.checked_count+' / serp_evidence='+evidence.length+'（最低'+SBOS_THRESHOLDS.SERP_CHECK_DEPTH+'件）');
    if(checked!==evidence.length) throw new Error('checked_count と serp_evidence 件数が一致しません: '+r.main_keyword+' / checked_count='+checked+' / evidence='+evidence.length);
    evidence.forEach((e,idx)=>{ const title=String(e&&e.title||'').trim(),url=String(e&&e.url||'').trim(); if(!title||!/^https?:\/\//i.test(url))throw new Error('検索証拠の title / url が不正です: '+r.main_keyword+' #'+(idx+1)); if(/\/(search|search\?|search\/)/i.test(url)&&/(hatenablog\.com|note\.com|qiita\.com|zenn\.dev|github\.com)/i.test(url))throw new Error('サイト内検索URLは個別SERP証拠として登録できません: '+r.main_keyword+' / '+url); });
    const band=String(r.reachable_band||'').toUpperCase().trim(); if(!['TOP10','TOP20','TOP30','OUT'].includes(band))throw new Error('reachable_band が不正です: '+r.main_keyword+' / '+String(r.reachable_band||'未設定'));
    const score=Number(r.blue_ocean_score); if(!isFinite(score)||score<0||score>100)throw new Error('不正なblue_ocean_score: '+r.blue_ocean_score+' / '+r.main_keyword);
    const finalDecision=r.hard_block===true?'BLOCK':sbosSerpGradeFromBand_(band); counts[finalDecision]++;
    row[5]=Math.round(score); row[6]=sbosIsNewSiteMode_()?'NOT_RUN':(['GREEN','YELLOW','PALE_PINK'].includes(finalDecision)?'PENDING':'NOT_RUN'); row[7]=String(r.search_intent||row[7]||''); row[8]=String(r.evidence_summary||''); row[11]=sbosIntentKey_(sbosNormalizeKeyword_(row[2])); row[12]=finalDecision; row[1]=sbosIsNewSiteMode_()?finalDecision:(['GREEN','YELLOW','PALE_PINK'].includes(finalDecision)?sbosStatusLabel_('CANNIBAL_PENDING'):finalDecision); row[27]=band; row[28]=String(payload.reviewed_at||payload.evaluated_at||sbosNow_()); row[29]=finalDecision==='BLOCK'?String(r.block_reason||'強制停止条件'):'';
    ...
    sbosAppendCandidateHistory_(row[2],finalDecision,band,payload.reviewed_at||payload.evaluated_at||sbosNow_(),'SERP_REVIEW',String(r.evidence_summary||'')); applied++;
  });
  if(!applied)throw new Error('Candidatesのキーワードと一致するSERP結果がありませんでした。');
  sh.getRange(2,1,data.length,30).setValues(data);
  const sourceName=source.sourceName||'AI Manual Review'; sbosWriteSerpReviewArchive_(payload,sourceName);
  const lastCompletedRequest=sbosGetState_('serp_last_completed_request_id')||'',thisRequestId=String(payload.request_id||''); if(thisRequestId&&thisRequestId!==lastCompletedRequest){sbosSetState_('serp_completed_cycles',Number(sbosGetState_('serp_completed_cycles')||0)+1);sbosSetState_('serp_last_completed_request_id',thisRequestId);sbosSetState_('serp_active_batch_keys','');}
  const excluded=sbosPruneRejectedCandidates_(false),progress=sbosGetSerpCycleProgress_(),shouldContinue=!progress.stopReached&&(progress.pending+progress.requested)>0,clustered=(progress.pending+progress.requested)===0?sbosCollapseCandidateIntentDuplicates_():0;
  sbosApplyCandidateFormatting_(); sbosSetState_('status',shouldContinue?SBOS_STATUS.SERP_RUNNING:SBOS_STATUS.SERP_REVIEW_IMPORTED); sbosSetState_('serp_result_source',source.sourceType||'UNKNOWN'); if(source.sourceId)sbosSetState_('serp_result_file_id',source.sourceId); sbosSetState_('serp_result_file_name',sourceName);
  const summary='SERP精査 '+progress.completedCycles+'/'+progress.maxCycles+'（GREEN＋YELLOW '+progress.yellowPlusTotal+'/'+progress.target+' / PALE PINK '+progress.palePink+'）'+(progress.stopReason?' / '+progress.stopReason:''); sbosSetHomeStatus_(summary); sbosSyncCandidatePool_(); sbosSaveCurrentBlogSession_();
...
function sbosWriteSerpReviewArchive_(payload, filename) {
  ...
  const headers=[['Request ID','Reviewed At','Rank','Main Keyword','BOS Grade','Reachable Band','Checked Count','Blue Ocean Score','Confidence','Hard Block','Block Reason','Exact/Near Exact Competitors','Big Site Pressure','Intent Gap','Personal Blog Chance','Suggested 4-Word Queries','Sources','Evidence Summary','Imported File']];
  ...
  const vals=payload.results.map(r=>[payload.request_id||'',payload.reviewed_at||payload.evaluated_at||'',r.rank||'',r.main_keyword||'',r.hard_block===true?'BLOCK':sbosSerpGradeFromBand_(r.reachable_band),r.reachable_band||'',r.checked_count||'',r.blue_ocean_score||'',r.confidence_percent||'',r.hard_block===true?'TRUE':'FALSE',r.block_reason||'',Array.isArray(r.exact_or_near_exact_competitors)?r.exact_or_near_exact_competitors.join(' | '):(r.exact_or_near_exact_competitors||''),r.big_site_pressure||'',r.intent_gap||'',r.personal_blog_chance||'',(r.suggested_four_word_queries||[]).join(' | '),(r.sources||[]).join(' | '),r.evidence_summary||'',filename||'']);
  if(vals.length)sh.getRange(sh.getLastRow()+1,1,vals.length,headers[0].length).setValues(vals); sh.setFrozenRows(1);if(!sh.isSheetHidden())sh.hideSheet();
}
...
function sbosParseJsonFromClaudeText_(text) {
  ...
  throw new Error('AIの結果を指定されたSIMS_BOS_SERP_REVIEW_RESULT JSONとして解析できませんでした。');
}

function sbosSerpGradeFromBand_(band) {
  const b = String(band || '').toUpperCase().trim();
  if (b === 'TOP10') return 'GREEN';
  if (b === 'TOP20') return 'YELLOW';
  if (b === 'TOP30') return 'PALE_PINK';
  return 'RED';
}
...
function sbosBuildClaudeSerpPrompt_(p) {
  return [
    ...
    'Task: Review every candidate using real, current web search evidence.',
    'Do not claim exact Google ranks unless your tool actually verifies them. Do not fabricate or infer missing results.',
    'Return only real pages you actually verified. Search-result listing pages must not be used as substitutes for individual organic results.',
    'For EACH candidate, collect at least ' + SBOS_THRESHOLDS.SERP_CHECK_DEPTH + ' verified results when possible and set checked_count to the actual number collected.',
    'Do not decide GREEN/YELLOW/PALE_PINK/RED yourself. Return reachable_band only: TOP10 / TOP20 / TOP30 / OUT.',
    'If the query lacks a coherent independent search intent, has no convincing demand signal, or is unsuitable for a standalone article, set hard_block=true and explain block_reason.',
    'For source=GENERATED_4WORD, confirm a real demand signal from search evidence, suggestions, Q&A, forums, or equivalent evidence.','',
    'SIMS grade mapping (reference only; do not output the grade):',
    '- TOP10 => GREEN',
    '- TOP20 => YELLOW',
    '- TOP30 => PALE_PINK',
    '- OUT => RED',
    '- hard_block=true => BLOCK regardless of band','',
    'Return contract: SIMS_BOS_SERP_REVIEW_RESULT_V2.',
...
function sbosSerpResultJsonSchema_() {
  const serpItem = {
    ...
    required:['title','url'],additionalProperties:true
  };
  const result = {
    type:'object',
    properties:{
      ...
      serp_evidence:{type:'array', items:serpItem},
      reachable_band:{type:'string', enum:['TOP10','TOP20','TOP30','OUT']},
      ...
      exact_or_near_exact_competitors:{type:'array'},
    ...
    required:['rank','main_keyword','checked_count','serp_evidence','reachable_band','confidence_percent','hard_block','block_reason','blue_ocean_score','search_intent','evidence_summary','exact_or_near_exact_competitors','big_site_pressure','intent_gap','personal_blog_chance','suggested_four_word_queries','sources'],
    additionalProperties:true
  };
  return {
    type:'object',
    properties:{
      format:{type:'string', const:'SIMS_BOS_SERP_REVIEW_RESULT_V2'},
    ...
    required:['format','request_id','results'],
    additionalProperties:true
  };
}
...
function sbosCreateCannibalReviewPackageFromEvidence(fileId) {
  ...
  try {
    ...
    const candidates = vals.filter(r => ['GREEN','YELLOW','PALE_PINK'].includes(String(r[12]||'').toUpperCase()) && sbosStatusCode_(r[1]) === 'CANNIBAL_PENDING').map(r => ({
      rank:Number(r[17])||0,main_keyword:r[2],words:Number(r[3])||0,serp_grade:String(r[12]||''),reachable_band:String(r[27]||''),blue_ocean_score:Number(r[5])||0,search_intent:r[7],evidence_summary:r[8],intent_key:r[11],review_mode:'FINAL_CANNIBAL_GATE'
    }));
...
function sbosBuildCreatorReferral_(v) {
  ...
  return [
    ...
    '## 最終判定', v[1] + ' / 到達見込み帯: ' + (v[27] || '未記録') + ' / Cannibalization: ' + v[6],'',
    '## 検索意図', v[7],'',
...
function sbosInitCandidatePool_() {
  ...
  const headers=['選択','対象サイト','メインキーワード','語数','現在判定','到達見込み帯','最終SERP評価日','aCreator状態','Source','判定根拠','Site Key','最終更新'];
...
function sbosInitCandidateHistory_(){const sh=SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATE_HISTORY);if(!sh)return;const h=['Site Key','Site','Keyword','Evaluated At','Stage','Status','Reachable Band','Evidence'];if(sh.getLastRow()===0){sh.getRange(1,1,1,h.length).setValues([h]).setFontWeight('bold');}if(!sh.isSheetHidden())sh.hideSheet();}
...
function sbosAppendCandidateHistory_(keyword,status,reachableBand,when,stage,evidence){const sh=SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATE_HISTORY);if(!sh)return;sh.appendRow([sbosPoolSiteKey_(),sbosGetSetting_('site_name')||'',String(keyword||''),String(when||sbosNow_()),String(stage||''),String(status||''),reachableBand||'',String(evidence||'')]);}
...
function sbosRegisterPoolReevaluation(rawText){
  ...
  if(requestId&&String(payload.request_id||'')!==requestId)throw new Error('request_idが一致しません。');
  if(!Array.isArray(payload.results)||payload.results.length!==1)throw new Error('再評価結果は1件だけ返してください。');
  const r=payload.results[0];if(sbosKeywordMatchKey_(r.main_keyword)!==sbosKeywordMatchKey_(kw))throw new Error('main_keywordが一致しません。');
  const checked=Number(r.checked_count),evidence=Array.isArray(r.serp_evidence)?r.serp_evidence:[];
  if(checked<SBOS_THRESHOLDS.SERP_CHECK_DEPTH||evidence.length<SBOS_THRESHOLDS.SERP_CHECK_DEPTH||checked!==evidence.length)throw new Error('検索証拠が不足、またはchecked_countと件数が一致しません。最低'+SBOS_THRESHOLDS.SERP_CHECK_DEPTH+'件必要です。');
  evidence.forEach((e,i)=>{const title=String(e&&e.title||'').trim(),url=String(e&&e.url||'').trim();if(!title||!/^https?:\/\//i.test(url))throw new Error('検索証拠のtitle/urlが不正です #'+(i+1));});
  const band=String(r.reachable_band||'').toUpperCase().trim();if(!['TOP10','TOP20','TOP30','OUT'].includes(band))throw new Error('reachable_bandが不正です。');
  const grade=r.hard_block===true?'BLOCK':sbosSerpGradeFromBand_(band),pool=SpreadsheetApp.getActive().getSheetByName(SBOS_SHEETS.CANDIDATE_POOL),selected=sbosGetSelectedPoolCandidate_();
  if(grade==='BLOCK'||grade==='RED'){pool.deleteRow(selected.row);}else{pool.getRange(selected.row,5).setValue(grade);pool.getRange(selected.row,6).setValue(band);pool.getRange(selected.row,7).setValue(payload.reviewed_at||payload.evaluated_at||sbosNow_());pool.getRange(selected.row,10).setValue(String(r.evidence_summary||''));pool.getRange(selected.row,12).setValue(sbosNow_());pool.getRange(selected.row,1).setValue(false);}
  sbosAppendCandidateHistory_(kw,grade,band,payload.reviewed_at||payload.evaluated_at||sbosNow_(),'SERP_REEVALUATION',String(r.evidence_summary||''));
  return {grade:grade,reachableBand:band,creatorEligible:['GREEN','YELLOW','PALE_PINK'].includes(grade)};
}

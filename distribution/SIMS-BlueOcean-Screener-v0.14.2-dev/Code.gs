...
function sbosBuildMenu_() {
  ...
  ui.createMenu('Blue Ocean Screener')
    ...
    .addItem('6. GREEN / YELLOW / PALE PINK候補をaCreatorで処理', 'sbosShowCreatorWorkflowDialog')
    .addItem('7. 候補・進捗を確認', 'sbosOpenCandidates')
    .addItem('8. 保存済み候補を見る', 'sbosOpenCandidatePool')
    .addItem('9. 保存済み候補を再評価', 'sbosReevaluateSelectedPoolCandidate')
    .addSeparator()
    .addSubMenu(
...
function sbosQueueSelectedCreatorCases() {
  ...
  sbosShowWorkflowResult_(
    'aCreator依頼待ちへ登録しました',
    '<b>登録:</b> '+selected.length+'件<br><br>案件は保存されています。今日すべて処理する必要はありません。次回はCandidatesで「依頼待ち」の案件をチェックし、「6. GREEN / YELLOW / PALE PINK候補をaCreatorで処理」から続けられます。',
    '7. 候補・進捗を確認',
    'sbosOpenCandidates'
  );
}
...
function sbosShowCreatorWorkflowDialog() {
  ...
  SpreadsheetApp.getUi().showModalDialog(html,'6. GREEN / YELLOW / PALE PINK候補をaCreator処理');
}
...
function sbosEnsureLightweightHome_(force) {
  ...
  if (needsBuild) {
    ...
    const rows = [
      ...
      ['標準フロー','','','','','','','','',''],
      ['1 対象サイト設定','2 キーワード読込','3 候補探索','4 SERP精査','5 カニバリ精査','6 aCreator処理','7 候補確認','8 Candidate Pool','9 再評価',''],
      ['','','','','','','','','',''],
      ['','','','','','','','','',''],
      ['色の見方','','','','','','','','',''],
      ['GREEN：推定1〜10位','','YELLOW：推定11〜20位','','PALE PINK：推定21〜30位','','RED：31位以下','','',''],
...
function sbosInitCandidatePool_() {
  ...
  sh.setColumnWidth(1,65);sh.setColumnWidth(2,160);sh.setColumnWidth(3,360);sh.setColumnWidth(5,110);sh.setColumnWidth(6,110);sh.setColumnWidth(7,150);sh.setColumnWidth(8,120);sh.setColumnWidth(10,420);try{sh.hideColumns(11);}catch(e){}
}

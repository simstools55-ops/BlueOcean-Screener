...
v0.14.9 は、Claude用ZIP作成ボタンが反応しないSERPダイアログのJavaScriptエスケープ不具合を修正しています。SERP精査仕様はv0.14.8を継承し、実在確認できた検索証拠を最低5件目安で評価します。対象サイト自身の記事がSERP証拠に含まれる候補は「既存記事確認済み」として新規記事候補から除外します。GREEN＋YELLOW 10件または5サイクル完了で停止し、aCreator依頼は引き続き1件ずつ処理します。

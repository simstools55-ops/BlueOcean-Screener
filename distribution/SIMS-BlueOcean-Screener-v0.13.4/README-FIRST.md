...
- Package作成時の全シート再初期化を廃止。
- CandidatesのREQUESTED更新を1件ずつではなく一括更新。
- 候補確認・依頼生成・ZIP生成・Drive保存・状態更新の処理時間を計測。
- エラー時に停止工程と経過時間を表示。
- ボタン押下直後に大きな処理中オーバーレイを表示。
- ブラウザ描画後にApps Scriptを呼び、回転表示が先に見えるよう変更。
- 成功後に処理時間を表示し、Package作成ボタンを非表示化。
- 判定ロジックは変更していません。
